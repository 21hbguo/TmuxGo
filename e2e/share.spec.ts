...
test('share link stays read-only and closes after revocation', async ({ page, request }) => {
  const session = await ensureSession(request, `tmuxgo_share_${Date.now()}`)
  const created = await request.post(`${apiUrl}/api/shares`, { data: { hostId: 'local', sessionName: session.name, expiresInMinutes: 5 } })
  expect(created.ok()).toBeTruthy()
  const link = await created.json() as { id: string; token: string }
  const sentFrames: string[] = []
  page.on('websocket', (socket) => socket.on('framesent', (frame) => sentFrames.push(typeof frame === 'string' ? frame : frame.toString())))
  ...
  expect(sentFrames.map((frame) => { try { return JSON.parse(frame) } catch { return null } }).filter((frame) => frame?.type === 'input')).toEqual([])
