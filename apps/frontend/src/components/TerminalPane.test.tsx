...
describe('TerminalPane', () => {
  ...
  it('does not consume ime composition while keyboard paste fallback is pending', async () => {
    const requestPaste = vi.fn()
    const targetInput = vi.fn()
    window.addEventListener('tmuxgo-request-terminal-paste', requestPaste)
    const { container } = render(<TerminalPane sessionName="dev" onInput={vi.fn()} onResize={vi.fn()} />)
  ...
  it('does not consume normal typing while keyboard paste fallback is pending', async () => {
    const requestPaste = vi.fn()
    const targetInput = vi.fn()
    window.addEventListener('tmuxgo-request-terminal-paste', requestPaste)
    const { container } = render(<TerminalPane sessionName="dev" onInput={vi.fn()} onResize={vi.fn()} />)
    ...
    expect(customKeyHandler?.({ ctrlKey: true, metaKey: false, altKey: false, key: 'v' } as KeyboardEvent)).toBe(false)
    target.value = 'x'
    const typing = new InputEvent('input', { bubbles: true, cancelable: true, data: 'x', inputType: 'insertText' })
    target.dispatchEvent(typing)
    await sleep(220)
