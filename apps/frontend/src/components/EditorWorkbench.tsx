      ...
      <div className="min-h-0 flex-1 bg-bg-0">
        {activeEditor.loading ? <div className="flex h-full items-center justify-center text-sm text-text-3">Loading {activeEditor.name}...</div> : activeEditor.problem || activeEditor.binary || activeEditor.truncated ? (
          ...
          <textarea
            ...
            onChange={(event) => setEditorContent(activeEditor.id, event.target.value)}
            spellCheck={false}
            className="h-full w-full resize-none border-0 bg-bg-0 p-4 text-sm text-text-1 outline-none"
            style={{ fontFamily: preferences.fontFamily, fontSize: Math.max(12, preferences.fontSize) }}
          />
        )}
      </div>
    </section>
