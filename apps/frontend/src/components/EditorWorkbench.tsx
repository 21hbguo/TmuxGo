...
export function EditorWorkbench({ onSaveEditor, onOpenFile, onOpenFileAtPosition = async (file) => file.id, onCreateCompare }:{ onSaveEditor: (editor: FileEditorDocument) => Promise<void>; onOpenFile: (file: FileDocumentHandle) => Promise<string>; onOpenFileAtPosition?: (file: FileDocumentHandle, placement: DropPlacement) => Promise<string>; onCreateCompare: (source: FileDocumentHandle, targetId: string) => Promise<void> }) {
  ...
  const renderTab = (editor: FileEditorDocument, groupEditors: FileEditorDocument[], groupId: string) => (
    <div key={editor.id} className={`group relative flex h-7 w-fit min-w-24 max-w-44 shrink-0 items-center border-r border-[rgba(255,255,255,0.04)] ${editor.id === activeEditor?.id ? 'bg-bg-0' : 'bg-bg-1/80'}`}>
      ...
      }} onClick={() => setActiveEditor(editor.id)} className={`flex min-w-0 flex-1 items-center gap-1.5 px-2 py-1.5 text-body ${editor.id === activeEditor?.id ? 'text-text-1' : 'text-text-2 hover:text-text-1'}`}>
  ...
  const renderTabStrip = (editors: FileEditorDocument[], groupId: string) => <div data-testid={`editor-group-${getLegacyGroupName(groupId)}`} data-editor-group-id={groupId} className={`tmuxgo-scrollbar-subtle relative flex min-h-7 items-stretch overflow-x-auto border-b border-[var(--line)] bg-bg-1 ${tabDropTarget?.groupId === groupId ? 'ring-1 ring-accent/40 ring-inset' : ''}`} onDragOver={(event) => {
    ...
    <div className={`absolute inset-[24%] rounded-apple ${tabDropTarget.placement === 'center' ? 'bg-accent/12 border border-accent/40' : ''}`} />
  ...
  const renderPane = (groupId: string) => {
    ...
    return <div className="flex min-h-0 min-w-0 flex-1 flex-col">{renderTabStrip(groupEditors, groupId)}<button onClick={() => editor && setActiveEditor(editor.id)} className="relative min-h-0 min-w-0 flex-1 overflow-hidden text-left" onDragOverCapture={(event) => {
      ...
      <div className={`absolute inset-[28%] rounded-apple border border-dashed ${paneDropTarget.placement === 'center' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/35'}`} />
      <div className={`absolute inset-y-0 left-0 w-[22%] rounded-l-apple border border-dashed ${paneDropTarget.placement === 'left' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
      <div className={`absolute inset-y-0 right-0 w-[22%] rounded-r-apple border border-dashed ${paneDropTarget.placement === 'right' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
      <div className={`absolute inset-x-0 top-0 h-[22%] rounded-t-apple border border-dashed ${paneDropTarget.placement === 'top' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
      <div className={`absolute inset-x-0 bottom-0 h-[22%] rounded-b-apple border border-dashed ${paneDropTarget.placement === 'bottom' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
    ...
    <section data-editor-drop className="flex h-full min-h-0 flex-col bg-bg-0">
      ...
      <div className="relative min-h-0 flex-1 bg-bg-0" onDragOver={(event) => {
        ...
        {dropTarget && <div className="pointer-events-none absolute inset-[16px] z-20">
          <div className={`absolute inset-[28%] rounded-apple border border-dashed ${dropTarget === 'center' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/35'}`} />
          <div className={`absolute inset-y-0 left-0 w-[22%] rounded-l-apple border border-dashed ${dropTarget === 'left' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
          <div className={`absolute inset-y-0 right-0 w-[22%] rounded-r-apple border border-dashed ${dropTarget === 'right' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
          <div className={`absolute inset-x-0 top-0 h-[22%] rounded-t-apple border border-dashed ${dropTarget === 'top' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
          <div className={`absolute inset-x-0 bottom-0 h-[22%] rounded-b-apple border border-dashed ${dropTarget === 'bottom' ? 'border-accent bg-accent/12' : 'border-[var(--line)] bg-bg-1/30'}`} />
