...
const hostRows: any[] = [
  { id: 'local', name: 'Local Host', address: '127.0.0.1', user: 'root', port: 22, status: 'online' },
...
describe('SshPanel', () => {
  ...
  it('collapses and expands host groups', () => {
    render(<SshPanel />)
    fireEvent.click(screen.getByText('Remote Hosts'))
  ...
  it('opens the create host dialog and saves a new host', async () => {
    ...
    await waitFor(() => expect(mutateCreateHost).toHaveBeenCalledWith(expect.objectContaining({ id: 'new-box', name: 'New Box', address: '9.9.9.9', user: 'deploy', port: 22 })))
  })
  ...
  it('attaches to a session and closes the ssh panel', async () => {
    useSessionsMock.mockReturnValue({ data: [{ id: 'sess-1', name: 'main' }, { id: 'sess-2', name: 'logs' }] })
