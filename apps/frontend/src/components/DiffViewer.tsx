  ...
  return (
    <div className="h-full overflow-auto bg-bg-0 font-mono text-meta leading-[1.6]">
      <div className="sticky top-0 z-10 border-b border-[var(--line)] bg-bg-1 px-4 py-2">
        ...
        {staged && <span className="ml-2 rounded-apple bg-accent/20 px-1.5 py-0.5 text-caption text-accent">staged</span>}
      </div>
      <table className="w-full border-collapse">
        <tbody>
          {lines.map((line, i) => {
            if (line.type === 'header') {
              return <tr key={i}><td colSpan={3} className="bg-bg-2 px-4 py-0.5 text-meta text-text-3">{line.content}</td></tr>
            }
            if (line.type === 'hunk') {
              return <tr key={i}><td colSpan={3} className="bg-accent/5 px-4 py-0.5 text-meta text-accent">{line.content}</td></tr>
            }
