...
export function DiffViewer({ hostId, repoPath, filePath, staged }: { hostId: string; repoPath: string; filePath: string; staged?: boolean }) {
  const { t } = useTranslation()
  const { data, isLoading } = useGitDiff(hostId, repoPath, { filePath, staged })
  ...
  return (
    <div className="h-full overflow-auto bg-bg-0 font-mono text-[12px] leading-[1.6]">
      <div className="sticky top-0 z-10 border-b border-[var(--line)] bg-bg-1 px-4 py-2">
        <span className="text-sm text-text-1">{t('git.diffTitle', { file: filePath })}</span>
