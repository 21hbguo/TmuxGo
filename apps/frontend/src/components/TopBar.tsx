...
export function TopBar() {
  const { activeHostId, activeSessionId, hosts, sessions, setCommandPalette, desktopPanel, sidebarCollapsed, toggleSidebar, setDesktopPanel } = useConsoleStore()
    ...
    <>
      <header className="h-14 bg-bg-1 border-b border-[var(--line)] flex items-center px-4 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => { if (desktopPanel === 'files' && !sidebarCollapsed) toggleSidebar(); else setDesktopPanel('files') }} className={`rounded-lg px-3 py-1.5 text-sm transition-colors ${desktopPanel === 'files' && !sidebarCollapsed ? 'bg-accent/20 text-accent' : 'bg-bg-2 text-text-3 hover:text-text-1'}`}>
            Explorer
          </button>
          <ConnectionBadge />
          <button
