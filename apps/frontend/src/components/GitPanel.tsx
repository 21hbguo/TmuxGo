  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex h-10 shrink-0 items-center gap-2 border-b border-[var(--line)] px-3">
        ...
        {searchQuery && <button type="button" aria-label={t('git.clearSearch')} title={t('git.clearSearch')} onClick={() => setSearchQuery('')} className="tmuxgo-icon-button flex h-6 w-6 items-center justify-center rounded-apple text-text-3 hover:bg-bg-2 hover:text-text-1"><FiX aria-hidden="true" size={13} /></button>}
      ...
      <div className="border-b border-[var(--line)] px-3 py-2">
        {showCreate ? (
          <div className="flex gap-1">
            <input value={newBranchName} onChange={(e) => setNewBranchName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCreate()} placeholder={t('git.branchName')} className="flex-1 rounded-apple border border-[var(--line)] bg-bg-0 px-2 py-1 text-[12px] text-text-1 outline-none focus:border-accent" autoFocus />
      ...
      <div className="border-b border-[var(--line)] px-3 py-2">
        ...
        {repoPath && (
          <div className="mt-2 flex items-center gap-1">
            <button aria-label={t('git.switchRepo')} title={t('git.switchRepo')} onClick={() => setRepoPickerOpen((open) => !open)} className={`tmuxgo-icon-button flex h-11 w-11 items-center justify-center rounded-apple hover:bg-bg-2 hover:text-text-1 lg:h-7 lg:w-7 ${repoPickerOpen ? 'bg-bg-2 text-accent' : 'text-text-3'}`}><FiFolder aria-hidden="true" size={14} /></button>
            {gitState?.mode === 'locked' && (
              <button aria-label={t('git.followCurrentFile')} title={t('git.followCurrentFile')} onClick={() => activeHostId && resumeGitFollowEditor(activeHostId)} className="tmuxgo-icon-button flex h-11 w-11 items-center justify-center rounded-apple text-accent hover:bg-bg-2 hover:text-text-1 lg:h-7 lg:w-7"><FiLink aria-hidden="true" size={14} /></button>
            )}
            <button aria-label={t('git.fetch')} title={t('git.fetch')} disabled={fetch.isPending} onClick={() => activeHostId && fetch.mutate({ hostId: activeHostId, path: repoPath, remote: selectedRemote || undefined, prune: fetchPrune }, { onSuccess: () => { pushToast({ type: 'success', message: t('git.fetchSuccess') }); queryClient.invalidateQueries({ queryKey: ['git-status'] }) }, onError: (err) => pushToast({ type: 'error', message: err.message }) })} className="tmuxgo-icon-button flex h-11 w-11 items-center justify-center rounded-apple text-text-3 hover:bg-bg-2 hover:text-text-1 disabled:opacity-50 lg:h-7 lg:w-7"><FiRefreshCw aria-hidden="true" className={fetch.isPending ? 'animate-spin' : ''} size={14} /></button>
            <button aria-label={t('git.pull')} title={t('git.pull')} disabled={pull.isPending} onClick={() => activeHostId && pull.mutate({ hostId: activeHostId, path: repoPath, remote: selectedRemote || undefined, rebase: pullRebase }, { onSuccess: () => { pushToast({ type: 'success', message: t('git.pullSuccess') }); queryClient.invalidateQueries({ queryKey: ['git-status'] }) }, onError: (err) => pushToast({ type: 'error', message: err.message }) })} className="tmuxgo-icon-button flex h-11 w-11 items-center justify-center rounded-apple text-text-3 hover:bg-bg-2 hover:text-text-1 disabled:opacity-50 lg:h-7 lg:w-7"><FiDownload aria-hidden="true" size={14} /></button>
            <button aria-label={t('git.push')} title={t('git.push')} disabled={push.isPending} onClick={() => activeHostId && push.mutate({ hostId: activeHostId, path: repoPath, remote: selectedRemote || undefined, branch: pushSetUpstream ? status?.branch : undefined, force: pushForce, setUpstream: pushSetUpstream }, { onSuccess: () => { pushToast({ type: 'success', message: t('git.pushSuccess') }); queryClient.invalidateQueries({ queryKey: ['git-status'] }) }, onError: (err) => pushToast({ type: 'error', message: err.message.includes('rejected') ? t('git.pushRejected') : err.message }) })} className="tmuxgo-icon-button flex h-11 w-11 items-center justify-center rounded-apple text-text-3 hover:bg-bg-2 hover:text-text-1 disabled:opacity-50 lg:h-7 lg:w-7"><FiUpload aria-hidden="true" size={14} /></button>
            <button aria-label={t('git.options')} title={t('git.options')} onClick={() => setGitOptionsOpen((open) => !open)} className={`tmuxgo-icon-button flex h-11 w-11 items-center justify-center rounded-apple hover:bg-bg-2 hover:text-text-1 lg:h-7 lg:w-7 ${gitOptionsOpen ? 'bg-bg-2 text-accent' : 'text-text-3'}`}><FiSettings aria-hidden="true" size={14} /></button>
          </div>
        )}
          ...
          {activeTab === 'status' && (
            <div className="border-t border-[var(--line)] p-3">
              <textarea
                ...
                rows={2}
                className="w-full resize-none rounded-apple border border-[var(--line)] bg-bg-0 px-2 py-1.5 text-[12px] text-text-1 outline-none focus:border-accent"
              />
