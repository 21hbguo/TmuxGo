      ...
      <div className="border-b border-[var(--line)] px-3 py-2">
        ...
        {repoPath && (
          <div className="mt-2 flex items-center gap-1">
            ...
            <button aria-label={t('git.fetch')} title={t('git.fetch')} disabled={fetch.isPending} onClick={() => activeHostId && fetch.mutate({ hostId: activeHostId, path: repoPath, remote: selectedRemote || undefined, prune: fetchPrune }, { onSuccess: () => { pushToast({ type: 'success', message: t('git.fetchSuccess') }); queryClient.invalidateQueries({ queryKey: ['git-status'] }) }, onError: (err) => pushToast({ type: 'error', message: err.message }) })} className="tmuxgo-toolbar-icon tmuxgo-toolbar-icon--lg lg:h-7 lg:w-7"><FiRefreshCw aria-hidden="true" className={fetch.isPending ? 'animate-spin' : ''} size={14} /></button>
            ...
            <button aria-label={t('git.push')} title={t('git.push')} disabled={push.isPending} onClick={() => activeHostId && push.mutate({ hostId: activeHostId, path: repoPath, remote: selectedRemote || undefined, branch: pushSetUpstream ? status?.branch : undefined, force: pushForce, setUpstream: pushSetUpstream }, { onSuccess: () => { pushToast({ type: 'success', message: t('git.pushSuccess') }); queryClient.invalidateQueries({ queryKey: ['git-status'] }) }, onError: (err) => pushToast({ type: 'error', message: err.message.includes('rejected') ? t('git.pushRejected') : err.message }) })} className="tmuxgo-toolbar-icon tmuxgo-toolbar-icon--lg lg:h-7 lg:w-7"><FiUpload aria-hidden="true" size={14} /></button>
