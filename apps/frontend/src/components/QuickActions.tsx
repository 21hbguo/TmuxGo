...
function useQuickActionController() {
  ...
  const { refreshSnapshot, resolveFreshActivePaneId, optimisticallyToggleWindowZoom, discardOptimisticWindowZoom } =
  ...
  const startRepeat = useCallback(
    (action: () => void, delay = repeatDelay, interval = repeatInterval) => {
      stopRepeat()
      action()
  ...
  const armTouchRepeat = useCallback(
    (action: () => void, delay = repeatDelay, interval = repeatInterval) => {
      stopRepeat()
  ...
  const confirmCreateWindow = useCallback(
  ...
  const primaryButtons: ActionButtonDef[] = [
    ...
    {
      key: 'new-window',
      label: t('quick.newWindow'),
      onPress: () => void handleOpenNewWindowPrompt(),
      tone: 'accent',
    ...
    { key: 'esc', label: 'Esc', data: '\x1b' },
    {
      ...
      data: '\x1b[A',
      repeat: true,
      repeatDelay: verticalRepeatDelay,
      repeatInterval: verticalRepeatInterval,
    },
    ...
    {
      key: 'kill-pane',
      label: t('quick.killPane'),
      onPress: () => void handleKillPane(),
      tone: 'danger',
      disabled: !activeSessionId,
    },
  ]
  const attachButton: ActionButtonDef = {
    key: 'attach-mode',
    label: preferences.attachExclusive ? t('quick.attachExclusive') : t('quick.attachShared'),
    onPress: () => updatePreferences({ attachExclusive: !preferences.attachExclusive }),
    tone: 'accent',
  }
  const fullscreenButton: ActionButtonDef = {
    key: 'immersive-fullscreen',
    label: preferences.immersiveFullscreen ? t('quick.exitFullscreen') : t('quick.fullscreen'),
    tone: 'accent',
    onPress: async () => {
      try {
  ...
  return {
    ...
    setEditingShortcut,
    isMobile,
    pendingShortcutDeletes,
    setPendingShortcutDeletes,
    confirmDeleteShortcut,
    confirmKillOpen,
    setConfirmKillOpen,
    pendingKillPaneId,
    setPendingKillPaneId,
    confirmKillPane,
    newWindowPromptOpen,
    setNewWindowPromptOpen,
    newWindowName,
    setNewWindowName,
    confirmCreateWindow,
    sendKey,
...
function renderPanelButton(def: ActionButtonDef, controller: ReturnType<typeof useQuickActionController>) {
  ...
  return (
    <KeyCap
      ...
      onPress={send}
      title={def.label}
      className={def.busy ? 'animate-pulse' : undefined}
    >
      {def.label}
    </KeyCap>
  )
      ...
      onPointerDown={(e) => {
        ...
        if (def.repeat && (def.data || def.onPress)) {
          ...
          if (e.pointerType !== 'mouse') {
            armTouchRepeat(fire, def.repeatDelay, def.repeatInterval)
            return
          }
...
export function QuickActions({ mode = 'panel', onOpenFiles }: { mode?: QuickActionsMode; onOpenFiles?: () => void }) {
  ...
  const closeShortcutManage = () => {
    setManagingShortcuts(false)
    setSelectedShortcutIds([])
  }
  const toggleManageShortcuts = () => {
    if (!managingShortcuts) {
      setManagingShortcuts(true)
      return
    }
  ...
  const confirmDeleteSelectedShortcut = () => {
    confirmDeleteShortcut()
    closeShortcutManage()
  }
  if (mode === 'dock') {
      ...
      <>
        <div className="mobile-nav-landscape-hide relative z-40 flex-shrink-0 bg-bg-1 border-t border-[var(--line)]">
          <div
            ...
            onPointerCancelCapture={(e) => controller.finishDockGesture(e.pointerId)}
            onScroll={controller.trackDockScroll}
            onWheel={(e) => {
              e.preventDefault()
          ...
          >
            <div className="flex gap-1 p-1.5 w-max min-h-[40px] items-center" onContextMenu={(e) => e.preventDefault()}>
              ...
              <KeyCap
                variant="dock"
                aria-label={t('shortcut.manage')}
                title={t('shortcut.manage')}
                onPress={() => setDockManageOpen(true)}
              >
        ...
        <MobileBottomSheet
          open={dockManageOpen}
          onClose={() => setDockManageOpen(false)}
          ariaLabel={t('shortcut.manage')}
        >
          <div className="px-4 pt-4 pb-2 overflow-y-auto">
            ...
            {shortcuts.map((s) => (
              <div key={s.id} className="flex items-center gap-1 py-1">
                ...
                <button
                  type="button"
                  onClick={() => {
                    setDockManageOpen(false)
                    setEditingShortcut(s)
                    setShowModal(true)
                  }}
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-apple text-text-3 transition-colors hover:bg-accent/15 hover:text-accent focus-visible:bg-accent/15 focus-visible:text-accent"
                ...
                <button
                  type="button"
                  onClick={() => setPendingShortcutDeletes([s])}
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-apple text-text-3 transition-colors hover:bg-danger/15 hover:text-danger focus-visible:bg-danger/15 focus-visible:text-danger"
                  aria-label={t('shortcut.delete')}
            ...
            <button
              ...
              className="w-full px-2 py-1.5 rounded-apple text-xs transition-colors border border-dashed border-[var(--line)] text-text-3 hover:text-text-2 hover:border-accent/50 mt-1"
            >
              + {t('shortcut.add')}
            </button>
          </div>
        ...
        <ConfirmDialog
          open={pendingShortcutDeletes.length > 0}
          title={t('shortcut.deleteTitle')}
          message={
            pendingShortcutDeletes.length === 1
              ? t('shortcut.deleteConfirm', { label: pendingShortcutDeletes[0]?.label || '' })
              : t('shortcut.deleteConfirmMany', { count: pendingShortcutDeletes.length })
          }
          confirmLabel={t('common.confirm')}
          cancelLabel={t('common.cancel')}
          tone="danger"
          onCancel={() => setPendingShortcutDeletes([])}
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="border-t border-[var(--line)] pt-2">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={toggleManageShortcuts}
                className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-apple text-text-3 transition-colors focus-visible:bg-accent/15 focus-visible:text-accent ${managingShortcuts ? 'bg-accent/15 text-accent' : 'hover:bg-accent/15 hover:text-accent'}`}
                aria-label={managingShortcuts ? t('shortcut.batchDone') : t('shortcut.batchManage')}
                title={managingShortcuts ? t('shortcut.batchDone') : t('shortcut.batchManage')}
              >
                <FiCheckSquare aria-hidden="true" size={13} />
              </button>
              <button
                type="button"
                onClick={requestDeleteSelectedShortcut}
                disabled={!managingShortcuts || selectedShortcutIds.length === 0}
                className="flex h-6 w-6 shrink-0 items-center justify-center rounded-apple text-text-3 transition-colors hover:bg-danger/15 hover:text-danger focus-visible:bg-danger/15 focus-visible:text-danger disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-text-3"
                aria-label={t('shortcut.delete')}
                title={t('shortcut.delete')}
              >
                <FiTrash2 aria-hidden="true" size={13} />
            ...
            {shortcuts.map((s) => (
              <div key={s.id} className="group relative flex min-w-0 items-center">
                <KeyCap
                  variant="panel"
                  size="md"
                  repeat={!managingShortcuts && s.repeat === true}
                  repeatDelay={verticalRepeatDelay}
                  repeatInterval={verticalRepeatInterval}
                  onPress={() => {
                    if (managingShortcuts) {
                      toggleSelectShortcut(s.id)
                      return
                    }
                    runShortcut(s)
                  }}
                  title={describeShortcut(s)}
                  tone={
                    managingShortcuts
                      ? selectedShortcutIds.includes(s.id)
                        ? 'accent'
                        : undefined
                      : runningShortcutId === s.id
                        ? 'accent'
                        : undefined
                  }
                  className={`min-w-0 flex-1 truncate pl-7 pr-7 ${!managingShortcuts && runningShortcutId === s.id ? 'animate-pulse' : ''}`}
                >
                  {s.label}
                </KeyCap>
                <button
                  ...
                  className={`absolute left-0 top-1/2 flex h-7 w-7 shrink-0 -translate-y-1/2 items-center justify-center rounded-apple text-text-3 transition-colors hover:bg-accent/15 hover:text-accent focus-visible:bg-accent/15 focus-visible:text-accent ${managingShortcuts ? '' : 'opacity-0 group-hover:opacity-100 focus-within:opacity-100'}`}
                  aria-label={t('shortcut.edit')}
                  title={t('shortcut.edit')}
                >
                  <FiEdit2 aria-hidden="true" size={13} />
                ...
                <button
                  type="button"
                  onClick={() => setPendingShortcutDeletes([s])}
                  className={`absolute right-0 top-1/2 flex h-7 w-7 shrink-0 -translate-y-1/2 items-center justify-center rounded-apple text-text-3 transition-colors hover:bg-danger/15 hover:text-danger focus-visible:bg-danger/15 focus-visible:text-danger ${managingShortcuts ? '' : 'opacity-0 group-hover:opacity-100 focus-within:opacity-100'}`}
                  aria-label={`${t('shortcut.delete')} ${s.label}`}
                  title={t('shortcut.delete')}
                >
                  <FiTrash2 aria-hidden="true" size={13} />
                </button>
      ...
      <button
        ...
        className="w-full px-2 py-1.5 rounded-apple text-xs transition-colors border border-dashed border-[var(--line)] text-text-3 hover:text-text-2 hover:border-accent/50"
      >
        + {t('shortcut.add')}
      </button>
      {showModal && (
      ...
      <ConfirmDialog
        ...
        tone="danger"
        onCancel={() => setPendingShortcutDeletes([])}
        onConfirm={confirmDeleteSelectedShortcut}
      />
      <ConfirmDialog
        open={confirmKillOpen}
        title={t('quick.killTitle')}
