      ...
      <div className="flex h-full min-h-0 flex-col bg-bg-1">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <SessionSortableList
            ...
            getItemClassName={({ session, isDragging, isOverlay }) => `border-b border-[rgba(255,255,255,0.03)] ${batchMode ? selectedSessionIds.includes(session.id) ? 'bg-red-900/15' : '' : activeSessionId === session.id ? 'bg-bg-2/80' : ''} ${isDragging && !isOverlay ? 'opacity-40' : ''} ${isOverlay ? 'rounded-lg border border-accent bg-bg-1 shadow-[0_20px_48px_rgba(0,0,0,0.42)]' : ''}`}
            renderItem={({ session, dragHandleProps }) => (
              <div className="flex items-center gap-1 pr-2">
                {batchMode ? <button onClick={() => toggleBatchSession(session.id)} className={`ml-2 flex h-7 w-5 shrink-0 items-center justify-center rounded text-[11px] leading-none ${selectedSessionIds.includes(session.id) ? 'text-red-300' : 'text-text-3'} hover:bg-bg-0`}>{selectedSessionIds.includes(session.id) ? '☑' : '☐'}</button> : <button ref={dragHandleProps.ref} {...dragHandleProps.attributes} {...dragHandleProps.listeners} aria-label={t('sidebar.reorderSession')} title={t('sidebar.reorderSession')} className="ml-2 flex h-7 w-5 shrink-0 items-center justify-center rounded text-[11px] leading-none text-text-3 transition-colors hover:bg-bg-0 hover:text-text-1 touch-none">⋮⋮</button>}
                <button onClick={() => batchMode ? toggleBatchSession(session.id) : setActiveSession(session.id)} onDoubleClick={() => !batchMode && void handleRenameSession(session.id)} className={`min-w-0 flex-1 border-l-2 px-3 py-2 text-left ${batchMode ? selectedSessionIds.includes(session.id) ? 'border-red-400' : 'border-transparent hover:bg-bg-2/60' : activeSessionId === session.id ? 'border-accent' : 'border-transparent hover:bg-bg-2/60'}`}>
