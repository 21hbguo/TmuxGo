...
vi.mock('./TaskNotifications', () => ({ TaskNotifications: () => React.createElement('div') }))
...
describe('ConsoleLayout mobile files overlay stack', () => {
  ...
  it('closes file overlay before terminal keyboard on app back', async () => {
    const backSpy = vi.spyOn(window.history, 'back')
    const dismissSpy = vi.fn()
    window.addEventListener('tmuxgo-dismiss-keyboard', dismissSpy)
    render(React.createElement(ConsoleLayout, { initialIsMobile: true }))
    fireEvent.click(screen.getByText('open-files'))
    fireEvent.click(await screen.findByText('push-level'))
    ...
    window.removeEventListener('tmuxgo-dismiss-keyboard', dismissSpy)
  })
