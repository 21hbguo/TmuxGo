...
export function PromptDialog({ open, title, defaultValue = '', confirmLabel, cancelLabel, onConfirm, onCancel }: PromptDialogProps) {
  ...
  return <ModalPortal>
    <div className="fixed inset-0 z-[80] flex items-center justify-center tmuxgo-scrim p-4" onClick={confirming ? undefined : onCancel}>
      <div className="tmuxgo-glass tmuxgo-glass-dialog w-full max-w-md rounded-apple border p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-5 flex justify-end gap-2">
          <Button variant="ghost" size="sm" disabled={confirming} onClick={onCancel}>{cancelLabel}</Button>
          <Button variant="primary" size="sm" disabled={confirming} onClick={submit}>{confirmLabel}</Button>
        </div>
      </div>
    </div>
  </ModalPortal>
