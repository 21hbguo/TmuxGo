...
export function PluginView({ pluginId, viewId, mode = 'panel', onClose }: PluginViewProps) {
  ...
  return <section className={`tmuxgo-content-surface flex min-h-0 flex-col overflow-hidden ${mode === 'panel' ? 'shrink-0 border-r border-[var(--line)]' : 'h-full w-full'}`} style={mode === 'panel' ? { width } : undefined}>
    <header className="flex h-11 shrink-0 items-center gap-2 border-b border-[var(--line)] px-3"><span className="flex h-6 w-6 items-center justify-center rounded-apple bg-bg-2 text-accent"><FiBox size={13} /></span><div className="min-w-0 flex-1"><div className="truncate text-xs font-medium text-text-1">{view.title}</div><div className="truncate text-caption text-text-3">{plugin.manifest.name}</div></div><Button variant="ghost" size="icon-sm" onClick={onClose} aria-label={t('common.close')} title={t('common.close')}><FiX size={15} /></Button></header>
