...
describe('AddShortcutModal macro validation', () => {
  it('disables save while a text step is empty and enables after typing', async () => {
    ...
    renderModal({
      onSave,
      onClose: vi.fn(),
      initialShortcut: { id: 'x', label: 'X', steps: [{ type: 'text', text: '', appendEnter: false }] },
    })
    await act(async () => {})
  ...
  it('disables save while a wait step has no duration', async () => {
    renderModal({
      onSave: vi.fn(),
      onClose: vi.fn(),
      initialShortcut: { id: 'y', label: 'Y', steps: [{ type: 'wait', ms: undefined }] },
    })
    await act(async () => {})
  ...
  it('edits a macro keys step with a picker on mobile', async () => {
    ...
    renderModal({
      ...
      onClose: vi.fn(),
      isMobile: true,
      initialShortcut: { id: 'z', label: 'Z', steps: [{ type: 'keys', keys: '' }] },
    })
    await act(async () => {})
  ...
  it('re-syncs the key picker after reordering macro steps on mobile', async () => {
    ...
    await act(async () => {})
    const combos = () => screen.getAllByRole('combobox')
    expect(combos()[1].getAttribute('data-value')).toBe('A')
    expect(combos()[3].getAttribute('data-value')).toBe('B')
    fireEvent.click(screen.getAllByRole('button', { name: '上移' })[1])
    expect(combos()[1].getAttribute('data-value')).toBe('B')
    expect(combos()[3].getAttribute('data-value')).toBe('A')
  })
  it('saves repeat flag when hold-to-repeat is checked', async () => {
    ...
    renderModal({
      onSave,
      onClose: vi.fn(),
      initialShortcut: { id: 'r', label: 'R', steps: [{ type: 'keys', keys: 'PageUp' }] },
    })
    await act(async () => {})
  ...
  it('keeps repeat unchecked by default and restores it when editing', async () => {
    ...
    renderModal({
      onSave,
      onClose: vi.fn(),
      initialShortcut: { id: 'n', label: 'N', steps: [{ type: 'keys', keys: 'PageDown' }] },
    })
    await act(async () => {})
    ...
    renderModal({
      onSave,
      onClose: vi.fn(),
      initialShortcut: { id: 'e', label: 'E', steps: [{ type: 'keys', keys: 'PageUp' }], repeat: true },
    })
    await act(async () => {})
