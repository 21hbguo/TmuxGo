...
import { getAuthStatus, login, onAuthChange, refreshAuth } from '@/lib/auth'
import { useTranslation } from '@/i18n'

type GateState = 'loading' | 'login' | 'error' | 'ready'
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        if (!active) return
        if (!status.enabled || status.authenticated || await refreshAuth()) {
          setState('ready')
          return
        }
        setState('login')
