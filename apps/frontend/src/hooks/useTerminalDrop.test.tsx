...
describe('useTerminalDrop', () => {
  it('activates drop state on drag over and clears it when leaving container', () => {
    ...
    act(() =>
      result.current.handleDragLeave(createDragEvent({ relatedTarget: document.createElement('span') }), container),
    )
  ...
  it('opens upload dialog when files are dropped', () => {
    ...
    const { result } = renderHook(() => useTerminalDrop(onInput, openUploadDialog))
    act(() =>
      result.current.handleDrop(createDragEvent({ dataTransfer: { files: [file] } as unknown as DataTransfer })),
    )
    expect(openUploadDialog).toHaveBeenCalledWith({ files: [file], insertPaths: true, temporary: true })
  ...
  it('inserts quoted absolute path from dragged editor file', () => {
    ...
    const { result } = renderHook(() => useTerminalDrop(onInput, vi.fn(), focusTerminal))
