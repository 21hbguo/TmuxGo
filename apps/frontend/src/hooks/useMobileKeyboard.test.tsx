...
describe('useMobileKeyboard', () => {
  ...
  it('does not open keyboard from closed state via keep-alive markers', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.focusKeyboard).toBeTruthy())
    const button = document.createElement('button')
    button.setAttribute('data-keep-mobile-keyboard', 'true')
  ...
  it('forwards arrow keys from window capture when focus is outside the textarea', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.textarea).toBeTruthy())
    sendInputMock.mockClear()
  ...
  it('does not double-send arrow keys when the textarea itself is focused', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.textarea).toBeTruthy())
    const textarea = api.textarea as HTMLTextAreaElement
    textarea.focus()
    sendInputMock.mockClear()
    fireEvent.keyDown(textarea, { key: 'ArrowRight', keyCode: 39, which: 39 })
    expect(sendInputMock).toHaveBeenCalledTimes(1)
    expect(sendInputMock).toHaveBeenCalledWith('\x1b[C')
  })
  it('does not capture arrow keys while another input is focused', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.textarea).toBeTruthy())
  ...
  it('repeats backspace after the mobile keyboard delete key is held', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.textarea).toBeTruthy())
    vi.useFakeTimers()
    const textarea = api.textarea as HTMLTextAreaElement
  ...
  it('keeps deleting when the IME interleaves keyup between repeated keydowns', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.textarea).toBeTruthy())
    vi.useFakeTimers()
    const textarea = api.textarea as HTMLTextAreaElement
    ...
    act(() => vi.advanceTimersByTime(33))
    expect(sendInputMock).toHaveBeenCalledTimes(5)
    fireEvent.keyUp(textarea, { key: 'Backspace' })
    act(() => vi.advanceTimersByTime(400))
    const count = sendInputMock.mock.calls.length
    act(() => vi.advanceTimersByTime(1000))
    expect(sendInputMock).toHaveBeenCalledTimes(count)
  })
  it('deletes exactly once on a quick tap', async () => {
    render(<Harness />)
    await waitFor(() => expect(api.textarea).toBeTruthy())
    vi.useFakeTimers()
    const textarea = api.textarea as HTMLTextAreaElement
