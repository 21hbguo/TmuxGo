...
describe('api git error handling', () => {
  ...
  it('fetches protected binary resources with credentials', async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, _init?: RequestInit) => ({ ok: true, status: 200, blob: async () => blob }))
    vi.stubGlobal('fetch', fetchMock)
  ...
  it('starts a background host test', async () => {
    const fetchMock = vi.fn(async (_url: string, _init?: RequestInit) => ({ ok: true, status: 202, text: async () => JSON.stringify({ task: { id: 'task-1', type: 'host-test', title: 'Test host edge', status: 'running' } }) }))
    vi.stubGlobal('fetch', fetchMock)
