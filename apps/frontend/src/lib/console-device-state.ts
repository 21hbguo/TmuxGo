...
export type PersistedEditorMeta = Pick<FileEditorDocument, 'id' | 'hostId' | 'rootId' | 'rootLabel' | 'rootPath' | 'path' | 'name' | 'absolutePath' | 'language' | 'kind' | 'compareLeftId' | 'compareRightId'>
...
export function readLegacyConsoleState(): { openEditors: PersistedEditorMeta[]; activeEditorId: string | null; activeHostId: string | null } | null {
  if (typeof window === 'undefined') return null
  try {
    ...
    const openEditors = (rawEditors as unknown[]).filter((item): item is PersistedEditorMeta => !!item && typeof item === 'object' && typeof (item as Record<string, unknown>).id === 'string' && typeof (item as Record<string, unknown>).hostId === 'string' && typeof (item as Record<string, unknown>).rootId === 'string' && typeof (item as Record<string, unknown>).rootLabel === 'string' && typeof (item as Record<string, unknown>).rootPath === 'string' && typeof (item as Record<string, unknown>).path === 'string' && typeof (item as Record<string, unknown>).name === 'string' && typeof (item as Record<string, unknown>).absolutePath === 'string' && typeof (item as Record<string, unknown>).language === 'string').map((item): PersistedEditorMeta => ({ ...item, kind: item.kind === 'compare' ? 'compare' : 'file' }))
