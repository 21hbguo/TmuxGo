export const zh = {
  'common.refresh': '刷新',
  'auth.title': '登录 TmuxGo',
  'auth.subtitle': '终端工作台',
  'auth.username': '账号',
  'auth.password': '密码',
  'auth.submit': '登录',
  'auth.signingIn': '登录中...',
  'auth.invalidCredentials': '账号或密码错误',
  'auth.unavailable': '认证服务暂时不可用',
  'auth.retry': '重试',
  'auth.required': '此 Gateway 需要账号密码认证',
  'auth.currentPassword': '当前密码',
  'auth.newPassword': '新密码',
  'auth.passwordChange': '修改密码',
  'auth.passwordChanging': '正在修改密码...',
  'auth.passwordChangeTitle': '设置新密码',
  'auth.passwordChangeRequired': '必须修改默认密码后才能继续使用 TmuxGo。',
  'settings.security': '安全',
  'settings.securityPasswordTitle': '密码',
  'settings.securityPasswordDesc': '修改密码后，所有设备都会退出登录。',
  'settings.securityDevicesTitle': '已登录设备',
  'settings.securityDevicesDesc': '查看活动会话并撤销不熟悉的设备访问。',
  'settings.securityRevokeOthers': '退出其他设备',
  'settings.securityRevoke': '撤销',
  'settings.securityCurrentDevice': '当前设备',
  'settings.securityDevice': '设备',
  'settings.securityUnknownDevice': '未知设备',
  'settings.securityCreatedAt': '创建于 {value}',
  'settings.securityLastUsedAt': '最近使用 {value}',
  'settings.securityExpiresAt': '过期于 {value}',
  'settings.securityNoDevices': '没有活动设备',
  'settings.securityLoadFailed': '无法加载已登录设备',
  'settings.securityActionFailed': '无法更新设备访问权限',
  'settings.securityPasswordFailed': '无法修改密码',
  'settings.securityDeviceRevoked': '已撤销设备访问权限',
  'settings.securityOtherRevoked': '已退出 {count} 台其他设备',
  'settings.shareTitle': '只读共享链接',
  'settings.shareDesc': '为一个终端会话创建临时访问链接。',
  'settings.shareHost': '主机',
  'settings.shareLocalHost': '本地主机',
  'settings.shareSession': '会话',
  'settings.shareExpires': '过期时间',
  'settings.shareMinutes': '{value} 分钟',
  'settings.shareHours': '{value} 小时',
  'settings.shareDays': '{value} 天',
  'settings.shareCreate': '创建链接',
  'settings.shareRevoke': '撤销',
  'settings.shareEmpty': '没有共享链接',
  'settings.shareCopied': '共享链接已复制',
  'settings.shareCreated': '共享链接已创建',
  'settings.shareRevoked': '共享链接已撤销',
  'settings.shareExpiresAt': '过期于 {value}',
  'settings.shareRevokedAt': '已撤销 {value}',
  'settings.shareLoadFailed': '无法加载共享链接',
  'settings.shareActionFailed': '无法更新共享链接',
  // TopBar
  'search.placeholder': '搜索...',
  'search.cmd': '⌘K',
  'install.title': '安装 TmuxGo',
  'install.desc': '安装后可从主屏幕全屏打开，使用更像原生 App。',
  'install.iosDesc': '在 Safari 点分享，再选"添加到主屏幕"，即可全屏打开。',
  'install.action': '立即安装',
  'install.gotIt': '我知道了',
  'install.later': '稍后',

  // Sidebar
  'sidebar.sessions': '会话',
  'sidebar.quickActions': '快捷操作',
  'sidebar.splitH': '横向分割',
  'sidebar.splitV': '纵向分割',
  'sidebar.newSession': '+ 新建会话',
  'sidebar.newAction': '新建',
  'sidebar.deleteSession': '结束会话',
  'sidebar.renameSession': '重命名会话',
  'sidebar.reorderSession': '拖动排序会话',
  'sidebar.deleteConfirm': '将结束会话 "{name}"，其中的程序会被终止，不能撤销。',
  'sidebar.deleteTitle': '结束会话',
  'sidebar.confirmDelete': '结束会话',
  'sidebar.batchDeleteAction': '批量结束',
  'sidebar.batchCancelAction': '取消批量',
  'sidebar.batchDeleteSelected': '结束选中',
  'sidebar.batchSelectAll': '全选',
  'sidebar.batchClearAll': '清空',
  'sidebar.batchSelectedCount': '已选 {count} 项',
  'sidebar.batchDeleteTitle': '批量结束会话',
  'sidebar.batchDeleteConfirm': '将结束选中的 {count} 个会话，其中程序会被终止，不能撤销。',
  'sidebar.batchDeleteSuccess': '已结束 {count} 个会话',
  'sidebar.windows': '{count} 个窗口',
  'hostSwitcher.label': '主机',

  // StatusBar
  'status.connected': '已连接',
  'status.attaching': '恢复终端',
  'status.reconnecting': '重连中',
  'status.disconnected': '已断开',
  'status.sessionSync': '同步',
  'status.sessionSyncStatus': '会话同步状态',
  'status.syncPending': '等待',
  'status.syncAge': '{seconds}秒',
  'status.syncFreshTitle': '会话上次成功同步于 {age} 前',
  'status.syncDelayedTitle': '会话同步延迟，上次成功于 {age} 前',
  'status.syncFailedTitle': '会话同步失败，上次成功于 {age} 前',
  'status.syncPendingTitle': '等待首次会话同步',
  'status.failed': '失败',
  'status.host': '主机',
  'status.hostScanStatus': '主机扫描状态',
  'status.scanFailed': '扫描失败',
  'status.scanFailedTitle': '远程主机 Agent 扫描失败，正在重试',
  'status.packets': '包',
  'status.netStats': 'WebSocket 收发统计',
  'status.netStatsTitle': '近 60 秒：发包 {tx} · 收包 {rx} · ping 丢包 {loss}',
  'status.netStatsValue': '发 {tx} · 收 {rx} · {loss}',

  // MobileNav
  'nav.sessions': '会话',
  'nav.windows': '窗口',
  'nav.panes': '面板',
  'nav.search': '搜索',
  'nav.settings': '设置',

  // MobileDrawer
  'drawer.sessions': '会话',
  'drawer.panes': '面板',
  'drawer.windows': '{count} 个窗口',
  'drawer.windowsTitle': '窗口',
  'drawer.noWindows': '没有窗口',
  'drawer.sessionName': '会话名称：',
  'drawer.renamePrompt': '重命名会话：',
  'drawer.expandPanes': '展开面板',
  'drawer.collapsePanes': '折叠面板',
  'drawer.paneCount': '{count} 个面板',

  // ConnectionBadge
  'badge.connected': '已连接',
  'badge.attaching': '恢复终端',
  'badge.reconnecting': '重连中',
  'badge.disconnected': '已断开',

  // CommandPalette
  'palette.placeholder': '搜索主机、会话、窗口...',
  'palette.hosts': '主机',
  'palette.sessions': '会话',
  'palette.windows': '{count} 个窗口',
  'palette.noResults': '未找到结果',
  'palette.navigate': '↑↓ 导航',
  'palette.select': '↵ 选择',
  'palette.close': 'Esc 关闭',

  // Settings
  'settings.title': '设置',
  'settings.general': '通用',
  'settings.appearance': '外观',
  'settings.session': '会话',
  'settings.plugins': '插件',
  'settings.auditLog': '审计日志',
  'settings.about': '关于',
  'settings.performance': '性能',
  'settings.performanceTitle': '系统性能健康',
  'settings.performanceDesc': '仅在本页打开时刷新。资源与流式指标以小卡片展示，关键指标附带最近约 1 分钟曲线。',
  'settings.performanceHost': '主机',
  'settings.performanceRefresh': '立即刷新',
  'settings.performanceUpdated': '最近更新',
  'settings.performanceIntervalLocal': '本地 1s 刷新',
  'settings.performanceIntervalRemote': '远程 2.5s 刷新',
  'settings.performanceResources': '主机资源',
  'settings.performanceDisks': '磁盘',
  'settings.performanceStream': '流式健康',
  'settings.performanceDependencies': '依赖',
  'settings.performanceActiveClients': '活跃连接',
  'settings.performanceProfile': '配置档',
  'settings.performanceOutputRate': '输出速率',
  'settings.performanceNetSent': '网络发送',
  'settings.performanceNetRecv': '网络接收',
  'settings.performanceNetWindow': '网络流量窗口',
  'settings.performanceNetDay': '今日',
  'settings.performanceNet24h': '近24小时',
  'settings.performanceNetSince': '已跟踪 {hours} 小时',
  'settings.performanceTotal': '累计',
  'settings.performanceSocketBuffer': 'Socket 缓冲',
  'settings.performanceBackpressure': '反压信号',
  'settings.performanceRate': '速率',
  'settings.performanceDropped': '丢弃字符',
  'settings.performanceFlushInterval': 'Flush 间隔',
  'settings.performanceMaxChars': '单次最大字符',
  'settings.performanceFlushes': 'Flush 次数',
  'settings.performanceChunks': '输出块',
  'settings.performanceInput': '输入消息',
  'settings.performanceDeferred': '延迟 Flush',
  'settings.performanceResync': 'Resync 完成/请求',
  'settings.performanceSuppressed': '背压抑制次数',
  'settings.performanceAttach': 'Attach 次数',
  'settings.performanceCompressFrames': '压缩帧',
  'settings.performanceCompressSaved': '压缩节省',
  'settings.performanceCellSnapshots': 'Cell 快照',
  'settings.performanceCellDiffs': 'Cell 差分',
  'settings.performanceCellFallback': 'Cell 回退 ANSI',
  'settings.performanceDepOk': '可用',
  'settings.performanceDepMissing': '缺失',
  'settings.performanceDepWarn': '缺失依赖：{list}',
  'settings.connection': '连接',
  'settings.reconnect': '重连',
  'settings.sessionContinuity': '会话持续化',
  'settings.sessionContinuityEnabled': '启用持续化',
  'settings.resumeOnReconnect': '重连后恢复',
  'settings.resumeOnNewDevice': '新设备恢复',
  'settings.resumePointCount': '恢复点数量',
  'settings.maxResumePoints': '最大恢复点',
  'settings.clearResumePoints': '清空恢复点',
  'settings.archiveEnabled': '归档终端输出',
  'settings.archiveCaptureMode': '捕获范围',
  'settings.archiveVisible': '当前可见内容',
  'settings.archiveHistory': '完整历史内容',
  'settings.archiveMaxSize': '单会话最大容量',
  'settings.archiveRetention': '保留天数',
  'settings.viewArchives': '查看归档',
  'settings.archives': '会话归档',
  'settings.archiveEmpty': '暂无会话归档',
  'settings.selectArchive': '选择一个归档',
  'settings.deleteArchive': '删除归档',
  'settings.archiveNoOutput': '未捕获到输出',
  'settings.hosts': '主机',
  'settings.hostId': '主机ID',
  'settings.hostNameOptional': '显示名称（可选）',
  'settings.hostAddress': '地址',
  'settings.hostUser': '用户',
  'settings.hostPort': '端口',
  'settings.hostPassword': '服务器密码（可选）',
  'settings.hostPasswordSaved': '已保存密码',
  'settings.hostPasswordKeep': '留空则保持原密码不变',
  'settings.hostPasswordEnv': '密码环境变量（可选）',
  'settings.hostPrivateKeyPath': 'SSH 私钥路径（可选）',
  'settings.hostPrivateKeyKeep': '留空则保持原私钥路径不变',
  'settings.hostPrivateKeySaved': '已配置私钥',
  'settings.hostJumpHost': '跳板机 / ProxyJump（可选）',
  'settings.hostJumpHostSaved': '跳板机：{host}',
  'settings.hostUseAgent': '使用 SSH Agent',
  'settings.hostAgentEnabled': '已启用 SSH Agent',
  'settings.hostKnownHostsPolicy': '主机密钥策略',
  'settings.hostKnownHostsStrict': '严格校验',
  'settings.hostKnownHostsAcceptNew': '接受新主机',
  'settings.hostKnownHostsOff': '关闭校验',
  'settings.hostGroups': '主机分组，使用逗号分隔',
  'settings.hostTags': '主机标签，使用逗号分隔',
  'settings.hostFavorite': '收藏主机',
  'settings.hostLatency': '延迟 {value} ms',
  'settings.hostConnectionMode': '连接方式：{mode}',
  'settings.hostConnectionMode.local': '本地',
  'settings.hostConnectionMode.ssh': 'SSH',
  'settings.hostConnectionMode.agent': 'Agent',
  'settings.hostAgentState': 'Agent {version}：{status}',
  'settings.hostAgentOnline': '在线',
  'settings.hostAgentOffline': '离线',
  'settings.hostAgentLastHeartbeat': '最近心跳 {value}',
  'settings.hostAgentReconnectCount': '重连次数 {count}',
  'settings.hostAgentDisconnectReason': '断线原因：{reason}',
  'settings.hostAgentDisconnectedAt': '断开于 {value}',
  'settings.hostNew': '新建主机',
  'settings.hostCreate': '新建主机',
  'settings.hostEdit': '编辑主机',
  'settings.hostSave': '保存主机',
  'settings.hostSaved': '主机已保存',
  'settings.hostSaveFailed': '保存主机失败',
  'settings.hostRemove': '删除',
  'settings.hostRemoved': '主机已删除',
  'settings.hostRemoveFailed': '删除主机失败',
  'settings.hostRemoveConfirmTitle': '删除主机',
  'settings.hostRemoveConfirmMessage': '确定删除主机 {name} 及其保存的连接信息吗？',
  'settings.hostTest': '测试',
  'settings.hostTestOk': '连接正常',
  'settings.hostTestFailed': '连接测试失败',
  'sshPanel.title': 'SSH',
  'sshPanel.newHost': '新建主机',
  'sshPanel.refresh': '刷新',
  'sshPanel.local': '本机',
  'sshPanel.remote': '远程主机',
  'sshPanel.agents': 'Agent',
  'sshPanel.empty': '没有配置主机',
  'sshPanel.edit': '编辑',
  'sshPanel.test': '测试',
  'sshPanel.attach': 'Attach',
  'sshPanel.delete': '删除',
  'sshPanel.details': '详情',
  'sshPanel.selectSession': '选择要连接的会话',
  'sshPanel.noSessions': '该主机没有会话',
  'sshPanel.testResult': '{host}：{title}',
  'sshPanel.testFailed': '连接测试失败',
  'sshPanel.hostId': '主机ID',
  'sshPanel.hostName': '显示名称（可选）',
  'sshPanel.hostAddress': '地址',
  'sshPanel.hostUser': '用户',
  'sshPanel.hostPort': '端口',
  'sshPanel.hostPassword': '服务器密码（可选）',
  'sshPanel.hostPasswordKeep': '留空则保持原密码不变',
  'sshPanel.hostPrivateKeyPath': 'SSH 私钥路径（可选）',
  'sshPanel.hostPrivateKeyKeep': '留空则保持原私钥路径不变',
  'sshPanel.hostJumpHost': '跳板机 / ProxyJump（可选）',
  'sshPanel.hostGroups': '主机分组，使用逗号分隔',
  'sshPanel.hostTags': '主机标签，使用逗号分隔',
  'sshPanel.hostFavorite': '收藏主机',
  'sshPanel.hostUseAgent': '使用 SSH Agent',
  'sshPanel.hostKnownHostsPolicy': '主机密钥策略',
  'sshPanel.hostKnownHostsStrict': '严格校验',
  'sshPanel.hostKnownHostsAcceptNew': '接受新主机',
  'sshPanel.hostKnownHostsOff': '关闭校验',
  'sshPanel.createTitle': '新建主机',
  'sshPanel.editTitle': '编辑主机',
  'sshPanel.save': '保存主机',
  'sshPanel.saved': '主机已保存',
  'sshPanel.saveFailed': '保存主机失败',
  'sshPanel.removeTitle': '删除主机',
  'sshPanel.removeMessage': '确定删除主机 {name} 及其保存的连接信息吗？',
  'sshPanel.removed': '主机已删除',
  'sshPanel.removeFailed': '删除主机失败',
  'sshPanel.detail.address': '地址',
  'sshPanel.detail.user': '用户',
  'sshPanel.detail.port': '端口',
  'sshPanel.detail.connectionMode': '连接方式',
  'sshPanel.detail.favorite': '收藏',
  'sshPanel.detail.groups': '分组',
  'sshPanel.detail.tags': '标签',
  'sshPanel.detail.useAgent': 'SSH Agent',
  'sshPanel.detail.forwardAgent': 'ForwardAgent',
  'sshPanel.detail.jumpHost': '跳板机',
  'sshPanel.detail.knownHostsPolicy': '主机密钥策略',
  'sshPanel.detail.latency': '延迟',
  'sshPanel.detail.lastError': '最近连接错误',
  'sshPanel.mode.local': '本地',
  'sshPanel.mode.ssh': 'SSH',
  'sshPanel.mode.agent': 'Agent',
  'sshPanel.policy.strict': '严格',
  'sshPanel.policy.acceptNew': '接受新主机',
  'sshPanel.policy.off': '关闭',
  'sshPanel.yes': '是',
  'sshPanel.no': '否',
  'sshPanel.entryConfig': 'SSH 入口配置',
  'sshPanel.userMap': '用户映射（TMUXGO_SSH_USER_MAP）',
  'sshPanel.userMapHint': '逗号分隔，例如 user=tmuxuser',
  'sshPanel.allowedHosts': '允许主机白名单（TMUXGO_SSH_ALLOWED_HOSTS）',
  'sshPanel.allowedHostsHint': '逗号分隔的主机 ID，空表示全部允许',
  'sshPanel.envHint': '保存在 gateway 环境变量/部署配置中，改动需重新部署生效。',
  'sshPanel.usage': '用法',
  'sshPanel.usageExample': 'ssh tmuxgo@gateway attach --host <host> --session <name>',
  'sshPanel.docPath': '部署文档',
  'sshPanel.docPathValue': 'apps/gateway/docs/ssh-gateway-deploy.md',
  'sshPanel.copy': '复制',
  'sshPanel.copied': '已复制到剪贴板',
  'sshPanel.removeAgentConfirm': '确定删除 agent {name} 的历史记录吗？',
  'sshPanel.jsonConfig': 'JSON 配置',
  'sshPanel.jsonConfigTitle': '编辑 JSON 配置',
  'sshPanel.jsonConfigHosts': '主机配置 (hosts.json)',
  'sshPanel.jsonConfigCredentials': '凭据配置 (host-credentials.json)',
  'sshPanel.jsonConfigSave': '保存',
  'sshPanel.jsonConfigCancel': '取消',
  'sshPanel.jsonConfigInvalid': 'JSON 格式无效，请检查后重试',
  'sshPanel.jsonConfigSaved': '配置已保存',
  'sshPanel.jsonConfigSaveFailed': '保存配置失败',
  'sshPanel.jsonConfigLoading': '正在加载配置…',
  'sshPanel.jsonConfigPath': '路径：{path}',
  'sshPanel.openConfig': '打开 SSH Config',
  'sshPanel.sshConfigTitle': 'SSH Config',
  'sshPanel.sshConfigSaved': 'SSH config 已保存',
  'sshPanel.sshConfigSaveFailed': '保存 SSH config 失败',
  'sshPanel.configManaged': '连接参数由 ssh config 管理，此处仅可编辑 overlay 字段',
  'sshPanel.detail.source': '来源',
  'sshPanel.detail.identityFile': '私钥文件',
  'sshPanel.quickAdd': '添加',
  'sshPanel.quickAddHint': 'user@host 或 host',
  'sshPanel.quickAddInvalid': '输入无效，格式为 user@host 或 host',
  'sshPanel.quickAddDone': '已将 {alias} 追加到 ssh config',
  'sshPanel.quickAddFailed': '添加主机失败',
  'sshPanel.resolve': '解析 (ssh -G)',
  'sshPanel.resolveFailed': '解析失败',
  'settings.autoReconnect': '自动重连',
  'settings.reconnectInterval': '重连间隔',
  'settings.notifications': '通知',
  'settings.agentNotificationsEnabled': '启用 Agent 通知',
  'settings.agentNotificationDuration': '自动消失时间',
  'settings.seconds': '{count} 秒',
  'settings.terminal': '终端',
  'settings.fontSize': '字体大小',
  'settings.fontFamily': '字体',
  'settings.cursorBlink': '光标闪烁',
  'settings.terminalPadding': '终端内边距',
  'settings.editor': '编辑器',
  'settings.editorWheelScrollLines': '滚轮行数',
  'settings.editorWheelScrollLines.auto': '默认',
  'settings.lines': '{count} 行',
  'settings.resetDefaults': '恢复默认',
  'settings.forceRefresh': '强制刷新',
  'settings.forceRefreshDesc': '重新加载浏览器页面，断开并重新连接全部会话。',
  'settings.theme': '主题',
  'settings.theme.dark': '深色',
  'settings.theme.light': '浅色',
  'settings.theme.highContrast': '高对比',
  'settings.theme.dracula': 'Dracula',
  'settings.theme.nord': 'Nord',
  'settings.theme.catppuccin': 'Catppuccin',
  'settings.theme.sage': 'Sage',
  'settings.immersiveFullscreen': '全屏模式',
  'settings.immersiveFullscreenDesc':
    '浏览器全屏沉浸。手机端会出现可拖拽返回球（点击返回，不退出全屏）；系统返回键仍会先退出全屏。',
  'settings.immersiveFullscreenFailed': '当前浏览器不支持全屏，或已被拦截',
  'settings.layout': '布局',
  'settings.sidebarPosition': '侧边栏位置',
  'settings.sidebarPosition.left': '左侧',
  'settings.sidebarPosition.right': '右侧',
  'settings.showStatusBar': '状态栏',
  'settings.showQuickActions': '快捷操作',
  'settings.language': '语言',
  'settings.auditDesc': '查看最近的管理操作',
  'settings.viewLog': '查看日志',
  'settings.uploadRateLimit': '上传速率限制',
  'settings.downloadRateLimit': '下载速率限制',
  'settings.transfer': '传输限速',
  'settings.aboutAppName': '应用',
  'settings.aboutDesc': '查看当前版本、构建信息和更新状态',
  'settings.aboutCurrentVersion': '当前版本',
  'settings.aboutCurrentBuild': '当前构建',
  'settings.aboutLatestVersion': '最新版本',
  'settings.aboutLatestBuild': '最新构建',
  'settings.aboutUpdateStatus': '更新状态',
  'settings.aboutUpdateCurrent': '已是最新',
  'settings.aboutUpdateAvailable': '发现新版本',
  'settings.aboutRefresh': '刷新页面即可更新到最新版本',
  'settings.aboutCopy': '复制版本信息',
  'settings.aboutCopied': '版本信息已复制',
  'settings.aboutChecking': '检查中',
  'settings.aboutLoadFailed': '版本信息加载失败',
  'settings.aboutUpdateBehind': '发现新版本（落后 {count} 个提交）',
  'settings.updateTitle': '应用更新',
  'settings.updateDesc': '从远端仓库拉取最新代码，自动重建并重启服务，tmux 会话保持不变。',
  'settings.updateAction': '一键更新',
  'settings.updateCheck': '检查更新',
  'settings.updateChecking': '检查中',
  'settings.updateCheckFailed': '检查更新失败',
  'settings.updateRunning': '更新中',
  'settings.updateStatus': '更新状态',
  'settings.updateDirty': '存在未提交的本地更改，更新可能失败',
  'settings.updateConfirmTitle': '确认更新 TmuxGo？',
  'settings.updateConfirmMessage': '将拉取远端最新代码并重建重启服务，连接会短暂中断后自动恢复。',
  'settings.updateSuccess': '已是最新版本',
  'settings.updateReloading': '更新完成，正在刷新…',
  'settings.updateFailed': '更新失败',
  'settings.restartTitle': '重启服务',
  'settings.restartDesc': '在应用内重建并重启 TmuxGo 的 frontend 和 gateway，保持 tmux 及 agent 状态不变。',
  'settings.restartAction': '重启并重建',
  'settings.restartStatus': '重启状态',
  'settings.restartStatusIdle': '空闲',
  'settings.restartStatusRunning': '执行中',
  'settings.restartStatusSuccess': '成功',
  'settings.restartStatusFailed': '失败',
  'settings.restartConfirmTitle': '确认重启 TmuxGo 服务？',
  'settings.restartConfirmMessage': '将执行 frontend、gateway 的 restart + rebuild，不会操作 tmux 或 agent。',
  'settings.restartSuccess': '重启并重建完成',
  'settings.restartFailed': '重启并重建失败',
  'tasks.title': '任务中心',
  'tasks.empty': '暂无最近任务',
  'tasks.cancel': '取消',
  'tasks.retry': '重试',
  'tasks.startedAt': '开始时间',
  'tasks.finishedAt': '结束时间',
  'tasks.error': '错误',
  'tasks.progress': '进度',
  'tasks.speed': '速度',
  'tasks.result': '结果',
  'tasks.logs': '日志',
  'tasks.noLogs': '暂无日志',
  'tasks.queued': '任务已开始',
  'tasks.notificationSuccess': '{title} 已完成',
  'tasks.notificationFailed': '{title} 失败：{message}',
  'tasks.status.idle': '空闲',
  'tasks.status.running': '执行中',
  'tasks.status.success': '成功',
  'tasks.status.error': '失败',
  'tasks.status.cancelled': '已取消',
  'plugins.localLink': '本地开发',
  'plugins.localPath': '插件目录绝对路径',
  'plugins.link': '链接插件',
  'plugins.linked': '已链接 {name}',
  'plugins.githubInstall': 'GitHub 安装',
  'plugins.preview': '预览安装',
  'plugins.install': '安装',
  'plugins.update': '更新',
  'plugins.rollback': '回滚',
  'plugins.rolledBack': '已回滚 {name}',
  'plugins.installed': '已安装 {name}',
  'plugins.trustWarning': '构建命令和插件 Action 将以当前系统用户身份运行，请确认来源与命令可信。',
  'plugins.installedTitle': '已安装插件',
  'plugins.installedCount': '{count} 个插件',
  'plugins.empty': '暂无已安装插件',
  'plugins.state.active': '运行中',
  'plugins.state.disabled': '已停用',
  'plugins.state.error': '异常',
  'plugins.toggle': '启用或停用插件',
  'plugins.uninstall': '卸载',
  'plugins.uninstallTitle': '卸载插件',
  'plugins.uninstallMessage': '确定卸载 {name}？插件状态数据也会被删除。',
  'plugins.actionComplete': '插件操作完成',
  'plugins.actionFailed': '插件操作失败',
  'plugins.viewUnavailable': '插件视图不可用',
  'plugins.permissions': '权限',
  'plugins.permission.actions.execute': '执行操作',
  'plugins.permission.host.context': '读取主机上下文',
  'plugins.permission.files.read': '读取文件',
  'plugins.permission.files.write': '写入文件',
  'plugins.grant': '授权',
  'plugins.revoke': '撤销',
  'plugins.permissionGranted': '插件权限已授权',
  'plugins.permissionRevoked': '插件权限已撤销',
  'plugins.grantPermissionTitle': '授权插件权限',
  'plugins.grantPermissionMessage': '允许 {name} {permission}？',
  'share.connecting': '正在连接',
  'share.readOnly': '只读',
  'share.invalid': '共享链接无效',
  'share.unavailable': '共享链接不可用',
  'share.disconnected': '已断开连接',

  // AuditLog
  'audit.title': '审计日志',
  'audit.desc': '最近的管理操作',
  'audit.time': '时间',
  'audit.user': '用户',
  'audit.action': '操作',
  'audit.target': '目标',
  'audit.result': '结果',
  'audit.close': '关闭',
  'audit.empty': '暂无审计记录',
  'audit.success': '成功',
  'audit.failure': '失败',
  'audit.createSession': '创建会话',
  'audit.deleteSession': '删除会话',
  'audit.splitPane': '分割面板',
  'audit.closePane': '关闭面板',
  'audit.renameSession': '重命名会话',

  // SessionTemplates
  'templates.title': '新建会话模板',
  'templates.desc': '为新会话选择模板',
  'templates.cancel': '取消',
  'templates.default': '默认',
  'templates.defaultDesc': '单窗口单面板',
  'templates.custom': '自定义',
  'templates.customDesc': '配置窗口数量、面板数量和布局',
  'templates.saved': '已保存模板',
  'templates.edit': '编辑',
  'templates.delete': '删除',
  'templates.name': '模板名称',
  'templates.description': '模板描述',
  'templates.paneIndex': '面板 {index}',
  'templates.command': '启动命令',
  'templates.cwd': '工作目录',
  'templates.env': '环境变量，每行一个',
  'templates.addPane': '添加面板',
  'templates.runOnce': '仅运行',
  'templates.saveAndCreate': '保存并创建',
  'templates.windowRange': '最多 {count} 个窗口',
  'templates.paneRange': '每窗口最多 {count} 个面板',
  'templates.windowIndex': '窗口 {index}',
  'templates.windowNamePlaceholder': 'win-{index}',
  'templates.addWindow': '添加窗口',
  'templates.split.horizontal': '横向分割',
  'templates.split.vertical': '纵向分割',
  'templates.layout.tiled': '平铺',
  'templates.layout.evenHorizontal': '平均横向',
  'templates.layout.evenVertical': '平均纵向',
  'templates.layout.mainHorizontal': '主横向',
  'templates.layout.mainVertical': '主纵向',
  'templates.development': '开发',
  'templates.developmentDesc': '编辑器 + 终端 + 服务器',
  'templates.monitoring': '监控',
  'templates.monitoringDesc': '多监控面板',
  'templates.training': 'ML 训练',
  'templates.trainingDesc': '训练 + 监控 + 日志',

  // PaneGrid
  'grid.noWindows': '没有打开的窗口',
  'grid.selectSession': '从侧边栏选择一个会话',
  'grid.addHost': '添加主机',
  'grid.createSession': '新建会话',
  'grid.selectRecent': '选择最近会话',
  'grid.control.owned': '可输入 · 本机控制',
  'grid.control.spectating': '旁观中 · 当前控制权不在本页',
  'grid.control.inactive': '旁观中 · 本页未激活',
  'grid.control.attaching': '正在附着终端…',
  'grid.control.reconnecting': '连接中断 · 正在重连',
  'grid.control.readonly': '只读分享',
  'grid.control.takeover': '接管',
  'grid.control.takeoverPending': '接管请求中…',
  'grid.input.pending': '待发输入 {count} 条',
  'grid.input.retry': '立即重试',
  'grid.input.clear': '清空待发输入',
  'grid.input.cleared': '待发输入已清空',

  // TerminalSearch
  'terminalSearch.placeholder': '搜索...',
  'quick.attachModeTitle': '附着模式',
  'quick.attachShared': '共享附着',
  'quick.attachExclusive': '独占附着',
  'quick.fullscreen': '全屏',
  'quick.exitFullscreen': '退出全屏',
  'immersive.backOrb': '返回',
  'immersive.backOrbHint': '点击返回/收起键盘 · 按住拖拽',
  'quick.copy': '复制',
  'quick.paste': '粘贴',
  'quick.newWindow': '新窗口',
  'quick.clearLine': '清除',
  'quick.deleteWord': '删词',
  'quick.zoom': '聚焦',
  'quick.killPane': '删面板',
  'quick.killConfirm': '确认关闭当前面板？',
  'quick.killTitle': '关闭面板',
  'common.cancel': '取消',
  'common.back': '返回',
  'common.pressBackAgainToExit': '已拦截返回，请用系统手势或主页键离开',
  'common.confirm': '确认',
  'common.close': '关闭',
  'common.loading': '加载中...',
  'common.loadFailed': '加载失败',
  'palette.newSession': '新建会话',
  'palette.splitHorizontal': '横向分割当前面板',
  'palette.splitVertical': '纵向分割当前面板',
  'palette.zoomPane': '聚焦当前面板',
  'palette.copySelection': '复制选中内容',
  'palette.pasteClipboard': '粘贴剪贴板',
  'palette.renameWindow': '重命名当前窗口',
  'palette.killWindow': '关闭当前窗口',
  'palette.openSettings': '打开设置',

  // ShortcutBar
  'shortcut.enter': '回车',
  'shortcut.clearLine': '清除',
  'shortcut.deleteWord': '删词',
  'shortcut.splitH': '横分',
  'shortcut.splitV': '竖分',
  'shortcut.zoom': '聚焦',
  'shortcut.kill': '关闭',

  // Custom Shortcuts
  'shortcut.custom': '自定义快捷键',
  'shortcut.manage': '管理快捷键',
  'shortcut.add': '添加快捷键',
  'shortcut.edit': '编辑快捷键',
  'shortcut.done': '完成',
  'shortcut.batchManage': '批量管理',
  'shortcut.batchDone': '完成管理',
  'shortcut.delete': '删除快捷键',
  'shortcut.deleteTitle': '删除快捷键',
  'shortcut.deleteConfirm': '确定删除快捷键“{label}”？',
  'shortcut.deleteConfirmMany': '确定删除选中的 {count} 个快捷键？',
  'shortcut.label': '按钮名称',
  'shortcut.stepKeys': '按键',
  'shortcut.stepText': '文本',
  'shortcut.stepWait': '等待',
  'shortcut.waitMs': '等待毫秒数',
  'shortcut.waitPlaceholder': '例如 500',
  'shortcut.addStep': '添加步骤',
  'shortcut.moveUp': '上移',
  'shortcut.moveDown': '下移',
  'shortcut.removeStep': '删除步骤',
  'shortcut.noSteps': '还没有步骤，点击下方添加',
  'shortcut.macroLabelPlaceholder': '例如：一键部署',
  'shortcut.textPlaceholder': '输入要快速发送到终端的内容',
  'shortcut.appendEnter': '发送后回车',
  'shortcut.repeatHold': '长按持续触发',
  'shortcut.recording': '按下组合键...',
  'shortcut.pressKeys': '点击录制',
  'shortcut.selectKey': '选择主键...',
  'shortcut.cancel': '取消',
  'shortcut.save': '保存',

  // PaneActions
  'pane.splitH': '横向分割',
  'pane.splitV': '纵向分割',
  'pane.fullscreen': '全屏',
  'pane.close': '关闭面板',
  'pane.splitSuccess': '面板分割完成',
  'pane.splitFailed': '分割失败',
  'pane.closed': '面板已关闭',
  'pane.closeFailed': '关闭失败',
  'pane.zoomFailed': '聚焦失败',
  'pane.switchFailed': '切换面板失败',
  'pane.noActive': '当前没有可操作的面板',
  'githubAuth.title': 'GitHub 登录',
  'githubAuth.desc': '打开 GitHub 设备登录页并输入这组验证码。',
  'githubAuth.code': '设备验证码',
  'githubAuth.open': '打开 GitHub',
  'githubAuth.copy': '复制验证码',
  'githubAuth.dismiss': '关闭',
  'githubAuth.copied': '已复制 GitHub 验证码',
  'githubAuth.copiedInApp': 'GitHub 验证码已保存到应用剪贴板',

  // CommandSnippets
  'snippets.title': '命令片段',
  'snippets.search': '搜索片段...',
  'snippets.name': '名称',
  'snippets.command': '命令',
  'snippets.add': '添加',
  'snippets.cancel': '取消',
  'snippets.addSnippet': '+ 添加片段',
  'snippets.listFiles': '列出文件',
  'snippets.diskUsage': '磁盘用量',
  'snippets.memoryUsage': '内存用量',
  'snippets.processList': '进程列表',
  'snippets.dockerContainers': 'Docker 容器',
  'snippets.gitStatus': 'Git 状态',
  'snippets.gitLog': 'Git 日志',

  // FilePanel
  'file.title': '文件',
  'split.title': '分屏会话',
  'split.primary': '主',
  'split.secondary': '副',
  'split.swap': '交换主副',
  'split.close': '退出分屏',
  'split.open': '打开分屏',
  'split.delete': '删除分屏',
  'split.created': '{primary} ⇄ {secondary} 分屏已创建',
  'file.upload': '上传',
  'file.addFavorite': '添加收藏',
  'file.removeFavorite': '删收藏',
  'file.clearExpanded': '全部收起',
  'file.upToParent': '返回上一级',
  'file.clearSearch': '清空搜索',
  'editor.dropOpen': '松开打开文件',
  'editor.emptyArea': '暂无打开的文件，从文件区拖入或双击打开',
  'editor.drop.center': '松开打开文件',
  'editor.drop.left': '松开后在左侧打开',
  'editor.drop.right': '松开后在右侧打开',
  'editor.drop.top': '松开后在上方打开',
  'editor.drop.bottom': '松开后在下方打开',
  'editor.compareUnavailable': '仅已加载的文本文件支持对比',
  'editor.compareMissing': '对比目标不可用',
  'file.searchName': '搜索文件名',
  'file.searchContent': '搜索文件内容',
  'file.all': '全部',
  'file.file': '文件',
  'file.dir': '目录',
  'file.type': '类型',
  'file.size': '大小',
  'file.modified': '修改时间',
  'file.permission': '权限',
  'file.dotfiles': '点文件',
  'file.sort': '排序',
  'file.sortName': '名称',
  'file.sortSize': '大小',
  'file.sortModified': '修改时间',
  'file.sortAsc': '升序',
  'file.sortDesc': '降序',
  'file.loading': '加载中...',
  'file.treeLoadFailed': '加载失败',
  'file.retryLoad': '重试',
  'file.largeDir': '目录较大，显示前 {count} 项',
  'file.emptyDir': '空目录',
  'file.favorites': '收藏目录',
  'file.favoritesHint': '收藏目录后会显示在这里，方便从 Workspace 或 Home 快速返回。',
  'file.favoriteDirs': '收藏目录',
  'file.noResults': '无结果',
  'file.loadFailed': '目录加载失败',
  'file.hostOffline': '主机离线，恢复后自动刷新',
  'file.tooManyResults': '结果过多，显示前 {count} 条',
  'file.insertPath': '插入路径到终端',
  'file.openEditor': '在编辑器中打开',
  'file.insertPathCtx': '插入路径',
  'file.copyName': '复制文件名',
  'file.copyRelativePath': '复制相对路径',
  'file.copyPath': '复制路径',
  'file.openPreview': '打开预览',
  'file.download': '下载',
  'file.downloadMany': '下载 {count} 项',
  'file.downloadFailed': '下载失败',
  'file.copy': '复制到...',
  'file.copyMany': '复制 {count} 项到...',
  'file.move': '移动到...',
  'file.moveMany': '移动 {count} 项到...',
  'file.targetDirectory': '目标目录：',
  'file.copied': '{name} 已复制',
  'file.copiedMany': '{count} 项已复制',
  'file.moved': '{name} 已移动',
  'file.movedMany': '{count} 项已移动',
  'file.moveFailedMany': '{count} 项移动失败',
  'file.markedCopy': '{name} 已复制，Ctrl+V 粘贴',
  'file.markedCut': '{name} 已剪切，Ctrl+V 移动',
  'file.refresh': '刷新',
  'file.loadMore': '加载更多（{shown}/{total}）',
  'file.transferFailed': '文件操作失败',
  'file.transferFailedMany': '{count} 项操作失败',
  'file.trash': '回收站',
  'file.moveToTrash': '移到回收站',
  'file.moveToTrashMany': '移动 {count} 项到回收站',
  'file.movedToTrash': '{name} 已移到回收站',
  'file.movedToTrashMany': '{count} 项已移到回收站',
  'file.restore': '恢复',
  'file.restored': '{name} 已恢复',
  'file.trashEmpty': '回收站为空',
  'file.undo': '撤销',
  'file.rename': '重命名',
  'file.newFile': '新建文件',
  'file.newFolder': '新建文件夹',
  'file.delete': '删除',
  'file.newFileName': '新文件名：',
  'file.newFolderName': '新文件夹名：',
  'file.fileCreated': '文件已创建',
  'file.folderCreated': '文件夹已创建',
  'file.createFailed': '创建失败',
  'file.renamePrompt': '重命名 {name}：',
  'file.renamed': '{name} 已重命名',
  'file.renameFailed': '重命名失败',
  'file.deleteConfirm': '删除{type} "{path}"？',
  'file.deleteConfirmMany': '删除选中的 {count} 项？',
  'file.deleted': '{name} 已删除',
  'file.deleteFailed': '删除失败',
  'file.deleteFailedMany': '{count} 项删除失败：{names}',
  'file.copyFailed': '复制失败',
  'file.pathCopied': '路径已复制',
  'file.nameCopied': '文件名已复制',
  'file.relativePathCopied': '相对路径已复制',
  'file.downloading': '正在下载 {name}',
  'file.onlyFilesDownload': '文件和目录均可下载',
  'file.previewSkippedLarge': '大文件预览已跳过',
  'file.previewSkippedBinary': '二进制文件预览已跳过',
  'file.previewUnavailable': '预览不可用',
  'file.largeFilePreview': '大文件暂仅支持预览模式',
  'file.binaryFilePreview': '二进制文件不可编辑',
  'file.nameCopiedInApp': '文件名已在应用内复制',
  'file.relativePathCopiedInApp': '相对路径已在应用内复制',
  'file.mobileEdit': '编辑',
  'file.mobileEditExit': '退出编辑',
  'file.mobileEditConfirmTitle': '进入编辑模式？',
  'file.mobileEditConfirmMessage': '编辑模式下，点按或滚动都可能修改文件，请注意避免误操作。',
  'file.mobileEditExitTitle': '有未保存的修改',
  'file.mobileEditExitMessage': '退出将丢失未保存的修改。',
  'file.mobileEditDiscard': '不保存退出',
  'file.mobileEditKeepEditing': '继续编辑',
  'file.mobileEditUnavailable': '该文件无法编辑',
  'file.mobileEditLoadFailed': '文件加载失败',
  'file.mobileEditSaveFailed': '保存失败',
  'file.mobileViewSource': '源码',
  'file.mobileViewRendered': '预览',
  'file.followActivePath': '跟随终端目录',
  'file.followSuspended': '已暂停跟随',
  'file.followManual': '手动浏览',
  'file.followResume': '恢复跟随',
  'file.followActivePathHint': '跟随激活终端的工作目录',

  // PasteConfirmDialog
  'paste.title': '确认粘贴',
  'paste.manualTitle': '手动粘贴',
  'paste.manualDesc': '剪贴板权限获取失败，请粘贴到输入框后发送。',
  'paste.cancel': '取消',
  'paste.retryPermission': '重试权限',
  'paste.escapePaste': '转义粘贴',
  'paste.send': '发送',

  // UploadConfirmDialog
  'upload.title': '确认上传',
  'upload.file': '{count} 个文件',
  'upload.renameConflict': '冲突时重命名',
  'upload.root': '根目录',
  'upload.directory': '目录',
  'upload.target': '目标路径',
  'upload.resolving': '正在解析默认目标...',
  'upload.filesLabel': '文件',
  'upload.insertPaths': '上传后将路径插入终端',
  'upload.cancel': '取消',
  'upload.starting': '准备中...',
  'upload.upload': '上传',
  'upload.failed': '上传失败',
  'upload.uploaded': '已上传 {count} 个文件',
  'upload.resolveFailed': '解析上传目标失败',

  // UploadQueue
  'uploadQueue.title': '上传队列',
  'uploadQueue.clean': '清理',
  'uploadQueue.failed': '失败',
  'uploadQueue.done': '完成',
  'uploadQueue.queued': '排队中',
  'uploadQueue.close': '关闭',

  // ActivityBar
  'activity.sessions': '会话',
  'activity.explorer': '资源管理',
  'activity.search': '搜索',
  'activity.settings': '设置',
  'activity.ssh': 'SSH',

  // Favorites
  'favorites.title': '收藏',
  'favorites.recent': '最近',
  'favorites.noFavorites': '暂无收藏',
  'favorites.noRecent': '暂无最近项',
  'favorites.clearAll': '清空',

  // TerminalDock
  'dock.terminal': '终端',
  'dock.full': '全屏',
  'dock.overlay': '浮层 {height}px',

  // EditorWorkbench
  'editor.find': '查找',
  'editor.clear': '清空',
  'editor.back': '返回',
  'editor.forward': '前进',
  'editor.definition': '跳转到定义',
  'editor.definitionNotFound': '未找到定义',
  'editor.definitionUnsupported': '当前语言不支持定义跳转',
  'editor.definitionSearching': '正在搜索定义…',
  'editor.preview': '预览',
  'editor.syncPreview': '同步预览',
  'editor.saving': '保存中...',
  'editor.save': '保存',
  'editor.saved': '已保存',
  'editor.saveFailedKept': '保存失败，修改仍保留',
  'editor.loading': '加载 {name} 中...',
  'editor.binaryNotEditable': '二进制文件不可编辑',
  'editor.largePreviewOnly': '大文件暂仅支持预览模式',
  'editor.closeConfirm': '关闭 {name} 前是否保存？',
  'editor.readOnly': '此文件为只读。',
  'editor.nothingToPreview': '没有可预览的内容。',

  // DesktopWorkbench
  'desktop.largePreviewMode': '大文件暂仅支持预览模式',
  'desktop.binaryNotEditable': '二进制文件不可编辑',
  'desktop.directoryNotEditable': '目录无法在编辑器中打开',
  'desktop.openFailed': '打开失败',
  'desktop.saveFailed': '保存失败',

  // PaneNotifications
  'notification.title': '通知',
  'notification.clearAll': '清空',
  'notification.empty': '暂无通知',
  'notification.unwatch': '取消监控',
  'notification.watch': '监控通知',
  'agent.status.idle': '空闲',
  'agent.status.working': '工作中',
  'agent.status.blocked': '等待处理',
  'agent.status.done': '已完成',
  'agent.status.unknown': '未知',
  'agent.notification.blocked': '{agent} 在 {session} 等待处理',
  'agent.notification.done': '{agent} 在 {session} 已完成',
  'agent.notification.permission': '{agent} 在 {session} 等待权限确认',
  'agent.notification.question': '{agent} 在 {session} 等待你的回答',
  'agent.notification.failed': '{agent} 在 {session} 执行失败',
  'agent.notification.ended': '{agent} 在 {session} 已退出',
  'agent.notification.disconnected': '{agent} 在 {session} 已断开',

  // AppVersionGuard
  'version.newAvailable': '有新版本可用，刷新页面更新。',

  // WindowList/WindowTabs
  'window.reorderFailed': '排序失败',
  'window.switchFailed': '切换窗口失败',
  'window.createTitle': '新建窗口',
  'window.created': '窗口 {name} 已创建',
  'window.createFailed': '新建窗口失败',
  'window.createMissingSession': '请先选择会话',
  'window.batchMode': '批量',
  'window.batchCancel': '退出批量',
  'window.batchSelectAll': '全选',
  'window.batchClearAll': '清空',
  'window.batchDeleteSelected': '删除 ({count})',
  'window.batchDeleteTitle': '批量删除窗口',
  'window.batchDeleteConfirm': '确认删除 {count} 个窗口？',
  'window.batchDeleteSuccess': '已删除 {count} 个窗口',
  'window.batchDeletePartial': '部分删除：{success} 个成功，{failed} 个失败',
  'window.batchDeleteFailed': '{count} 个窗口删除失败',
  'window.cannotDeleteActive': '当前活动窗口不可删除',

  // TerminalPane
  'terminal.dropUpload': '拖放文件以上传',
  'terminal.viewingHistory': '正在查看历史 · 返回缓冲区底部',
  'terminal.linkOpenBlocked': '浏览器阻止了新窗口，请允许弹窗后重试',
  'terminal.dock.bottom': '底部',
  'terminal.dock.left': '左侧',
  'terminal.dock.right': '右侧',
  'terminal.dockHint': '终端区位置：{position}（点击展开选项，拖动此栏到工作区边缘停靠）',

  // Clipboard
  'clipboard.copyFailed': '复制失败',
  'clipboard.unavailable': '剪贴板不可用，已保存到应用',
  'clipboard.empty': '剪贴板为空',
  'clipboard.pasteFailed': '粘贴失败',
  'clipboard.pastedFromApp': '已从应用剪贴板粘贴',
  'clipboard.meta.multiline': '多行',
  'clipboard.meta.controlChars': '控制字符',
  'clipboard.meta.appClipboard': '应用剪贴板',
  'clipboard.meta.unavailable': '剪贴板不可用',

  // Session CRUD
  'session.created': '会话 {name} 已创建',
  'session.deleted': '会话 {name} 已结束',
  'session.renamed': '会话 {from} 已重命名为 {to}',
  'session.requestFailed': '请求失败',
  'session.loadFailed': '会话加载失败',
  'session.createTitle': '新建会话',
  'session.createAction': '创建',
  'session.workspace': '工作区',
  'session.noWorkspace': '未选择（使用默认目录）',
  'session.selectWorkspace': '选择工作区',
  'session.changeWorkspace': '更改',
  'session.hidePicker': '收起',
  'session.browseDirectory': '浏览目录…',
  'session.selectThisFolder': '选择此目录',
  'session.workspaceHintSelected': '新建 pane 将自动 cd 到该工作区',
  'session.workspaceHintNone': '不选工作区时使用 tmux 默认目录',
  'session.workspacePathMissing': '工作区路径无效，请重新选择工作区',
  'common.retry': '重试',

  // Workspaces
  'workspace.unclassified': '未分类',
  'workspace.createNew': '从目录新建工作区…',
  'workspace.createTitle': '新建工作区名称',
  'workspace.clear': '清除',
  'workspace.newSession': '从工作区新建会话',
  'workspace.rename': '重命名工作区',
  'workspace.renamePrompt': '工作区名称',
  'workspace.renamed': '工作区已重命名为 {name}',
  'workspace.template': '默认模板',
  'workspace.templateUpdated': '工作区 {name} 的模板已更新',
  'workspace.delete': '删除工作区',
  'workspace.deleteTitle': '删除工作区',
  'workspace.deleteConfirm': '删除工作区 {name}？其中的会话将保留并归入未分类。',
  'workspace.deleteAction': '删除',
  'workspace.deleted': '工作区 {name} 已删除',
  'workspace.requestFailed': '操作失败',
  'workspace.current': '当前工作区',
  'workspace.choose': '选择工作区',
  'workspace.add': '添加工作区',
  'workspace.directoryTitle': '选择工作区目录',
  'workspace.selectDirectory': '选择左侧目录',
  'workspace.openDirectory': '打开',
  'workspace.showHidden': '显示隐藏文件',
  'workspace.hideHidden': '隐藏隐藏文件',
  'mobile.quickSessionPin': '固定到快捷栏',
  'mobile.quickSessionUnpin': '取消固定',

  // CommandPalette extra
  'palette.switchWindow': '切换窗口：{name}',
  'palette.openFiles': '打开文件',
  'palette.toggleSessions': '切换会话面板',
  'palette.escToClose': 'Esc 关闭',
  'palette.actionFailed': '操作失败',

  // TopBar
  'topBar.sessions': '会话',
  'topBar.files': '文件',

  // MobileNav extra
  'nav.files': '文件',
  'nav.git': 'Git',
  'nav.off': '离线',
  'nav.more': '更多',
  'nav.compactBar': '精简底栏',
  'nav.fullBar': '完整底栏',
  'nav.inbox': '收件箱',

  // Agent inbox
  'inbox.title': '收件箱',
  'inbox.empty': '暂无消息',
  'inbox.unreadDot': '未读',
  'inbox.markAllRead': '全部已读',
  'inbox.markRead': '标为已读',
  'inbox.delete': '删除',
  'inbox.loadMore': '加载更多',
  'inbox.jump': '跳转',
  'inbox.view': '查看',
  'inbox.newMessage': '新消息',
  'inbox.targetMissing': '目标 pane 不存在或已关闭',
  'inbox.download': '下载',
  'inbox.loadMedia': '点击加载',
  'inbox.expired': '消息不存在或已过期',
  'inbox.noPreview': '选择一条消息预览',

  // FilePanel extra
  'file.inserted': '已插入 {name}',

  // Git
  'git.title': 'Git',
  'git.status': '状态',
  'git.history': '历史',
  'git.branches': '分支',
  'git.staged': '暂存区',
  'git.unstaged': '未暂存',
  'git.untracked': '未跟踪',
  'git.conflicted': '冲突',
  'git.useOurs': '使用当前',
  'git.useTheirs': '使用传入',
  'git.markResolved': '标记解决',
  'git.continueOperation': '继续 {operation}',
  'git.abortOperation': '中止 {operation}',
  'git.operationFailed': 'Git 操作失败',
  'git.options': 'Git 选项',
  'git.remote': '远程仓库',
  'git.prune': '清理远程引用',
  'git.rebasePull': '变基拉取',
  'git.forceWithLease': '安全强推',
  'git.setUpstream': '设置上游',
  'git.noFF': '保留合并提交',
  'git.noChanges': '没有更改',
  'git.noRepo': '未检测到 Git 仓库',
  'git.noActiveEditor': '当前没有激活的编辑器文件',
  'git.fileNotInRepo': '当前文件不在 Git 仓库中',
  'git.selectRepo': '搜索仓库或输入仓库路径',
  'git.noRepoResults': '未找到匹配仓库，请输入完整路径',
  'git.repoSearchFailed': '仓库搜索失败',
  'git.loadFailed': 'Git 数据加载失败',
  'git.commit': '提交',
  'git.commitMessage': '提交信息...',
  'git.commitSuccess': '提交成功',
  'git.commitFailed': '提交失败',
  'git.amend': '修改上次提交',
  'git.stage': '暂存',
  'git.unstage': '取消暂存',
  'git.stageAll': '暂存全部',
  'git.unstageAll': '取消暂存全部',
  'git.discard': '丢弃更改',
  'git.discardConfirm': '确定丢弃对 "{file}" 的更改？此操作不可撤销。',
  'git.discardTitle': '丢弃更改',
  'git.viewDiff': '查看差异',
  'git.branch': '分支',
  'git.ahead': '领先 {count} 个提交',
  'git.behind': '落后 {count} 个提交',
  'git.diffTitle': '差异: {file}',
  'git.diffEmpty': '没有差异',
  'git.detecting': '检测中...',
  'git.refresh': '刷新',
  'git.newBranch': '新建分支',
  'git.branchName': '分支名称',
  'git.checkout': '切换',
  'git.deleteBranch': '删除分支',
  'git.deleteBranchConfirm': '确定删除分支 "{name}"？',
  'git.merge': '合并',
  'git.mergeInto': '合并到当前分支',
  'git.mergeSuccess': '合并成功',
  'git.mergeFailed': '合并失败',
  'git.createBranch': '创建分支',
  'git.createBranchSuccess': '分支已创建',
  'git.checkoutSuccess': '已切换到 {branch}',
  'git.currentBranch': '当前分支',
  'git.loadMore': '加载更多',
  'git.author': '作者',
  'git.date': '日期',
  'git.fetch': '拉取远程',
  'git.pull': '拉取',
  'git.push': '推送',
  'git.fetchSuccess': '拉取成功',
  'git.pullSuccess': '拉取成功',
  'git.pushSuccess': '推送成功',
  'git.pushRejected': '推送被拒绝',
  'git.modeFollowingFile': '跟随编辑器或终端',
  'git.modeLocked': '已锁定仓库',
  'git.followCurrentFile': '跟随当前文件',
  'git.pinnedRepos': '固定仓库',
  'git.recentRepos': '最近仓库',
  'git.searchCommits': '查找提交、作者、分支或标签',
  'git.clearSearch': '清除查找',
  'git.openRepo': '打开仓库',
  'git.workingTreeChanges': '工作区更改 ({count})',
  'git.workingTreeSummary': '已暂存 {staged} · 未暂存 {unstaged} · 未跟踪 {untracked} · 冲突 {conflicted}',
  'git.switchRepo': '切换仓库',

  'vnc.title': '桌面',
  'vnc.fullscreen': '全屏',
  'vnc.minimize': '最小化',
  'vnc.windowed': '窗口化',
  'vnc.fullView': '铺满视图',
  'vnc.moreActions': '更多操作',
  'vnc.connect': '连接',
  'vnc.disconnect': '断开',
  'vnc.connecting': '连接中…',
  'vnc.connected': '已连接',
  'vnc.disconnected': '已断开',
  'vnc.idle': '未连接',
  'vnc.disconnectUnclean': '连接中断',
  'vnc.connectTimeout': '连接超时，请检查网络或虚拟屏',
  'vnc.securityFailure': '安全协商失败',
  'vnc.credentialsRequired': '需要 VNC 认证',
  'vnc.username': '用户名',
  'vnc.password': '密码',
  'vnc.rememberPassword': '记住密码',
  'vnc.rotateHint': '横屏体验更佳',
  'vnc.landscapeFullscreen': '横屏全屏',
  'vnc.mobileKeyboard': '键盘输入',
  'vnc.mobileKeyboardPlaceholder': '输入内容发送到桌面…',
  'vnc.displays': '虚拟屏',
  'vnc.displaysLoading': '探测中…',
  'vnc.displayStart': '启动该虚拟屏',
  'vnc.displayStop': '关闭该虚拟屏',
  'vnc.displayNeedPassword': '需先在目标机运行 vncpasswd 设置 VNC 密码',
  'vnc.displayNoServer': '目标机未安装 Xvnc 系服务端（tigervnc 等），无法创建虚拟屏',
  'vnc.displayActionFailed': '操作失败',
  'vnc.displayDetected': '检测到运行中的虚拟屏 {display}',
  'vnc.customPort': '自定义端口',
  'vnc.viewOnly': '仅查看',
  'vnc.sendCad': '发送 Ctrl+Alt+Del',
  'vnc.paste': '粘贴剪贴板到远端',
  'vnc.clipboardDenied': '浏览器拒绝了剪贴板读取',
  'vnc.setupCheck': '检测 VNC 环境',
  'vnc.setupChecking': '检测中…',
  'vnc.setupInstall': '一键安装并启动 x11vnc',
  'vnc.setupStart': '启动 VNC 服务',
  'vnc.setupInstalling': '安装中…',
  'vnc.setupReady': 'VNC 服务已就绪，可重新连接',
  'vnc.setupManual': '请在目标机上手动执行：',
  'vnc.setupNeedSudo': '无免密 sudo，请手动执行：',
  'vnc.setupMacos': 'macOS 自带屏幕共享：系统设置 → 通用 → 共享 → 屏幕共享',
  'vnc.setupWayland': '检测到 Wayland 会话，请按 compositor 安装 wayvnc 或 gnome-remote-desktop',
  'vnc.setupUnsupported': '当前系统不支持自动安装',
  'vnc.setupFailed': '安装失败',
  'vnc.copyCommand': '复制命令',
  'vnc.copied': '已复制',
  'vnc.tuning': '画质调节',
  'vnc.quality': '画质',
  'vnc.compression': '压缩',
  'vnc.maxFps': '帧率上限',
  'vnc.preset.speed': '极速',
  'vnc.preset.balanced': '均衡',
  'vnc.preset.saver': '省流',
  'vnc.restorePrev': '恢复上次',
  'vnc.resolution': '远端分辨率',
  'vnc.resolution.off': '原生',
  'vnc.resolution.auto': '跟随窗口',
  'vnc.lossless': '无损画质',
  'vnc.losslessHint': '剔除 JPEG/H.264 有损编码，文字逐像素无损；照片/视频类画面流量会增大',
  'vnc.showStats': '显示统计',
} as const
