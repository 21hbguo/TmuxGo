...
function installLaunchdService(runtimeRoot) {
  ...
  const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0"><dict><key>Label</key><string>${serviceLabel}</string><key>ProgramArguments</key><array><string>${xmlEscape(process.execPath)}</string><string>${xmlEscape(server)}</string></array><key>WorkingDirectory</key><string>${xmlEscape(runtimeRoot)}</string><key>EnvironmentVariables</key><dict><key>NODE_ENV</key><string>production</string><key>PORT</key><string>3001</string><key>TMUXGO_HOST</key><string>127.0.0.1</string><key>PATH</key><string>${servicePath}</string><key>TMUXGO_FRONTEND_DIST</key><string>${xmlEscape(join(runtimeRoot, 'vendor', 'frontend'))}</string></dict><key>RunAtLoad</key><true/><key>KeepAlive</key><true/><key>StandardOutPath</key><string>${xmlEscape(join(logDir, 'gateway.log'))}</string><key>StandardErrorPath</key><string>${xmlEscape(join(logDir, 'gateway.log'))}</string></dict></plist>\n`
