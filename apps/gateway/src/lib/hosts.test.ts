...
test('migrates host credentials out of hosts.json with restricted permissions', async () => {
  ...
  await hosts.upsertRemoteHost({ id: 'remote', name: 'remote', address: 'remote.example', user: 'guo', privateKeyPath: '/home/guo/.ssh/id_ed25519', groups: ['research', 'gpu'], favorite: true, useAgent: false, jumpHost: 'jump@bastion.example', knownHostsPolicy: 'strict' })
  const updated = await hosts.getHostById('remote')
  const updatedCredentials = await hosts.getHostCredentials('remote')
  assert.deepEqual({ groups: updated?.groups, favorite: updated?.favorite, useAgent: updated?.useAgent, jumpHost: updated?.jumpHost, knownHostsPolicy: updated?.knownHostsPolicy }, { groups: ['research', 'gpu'], favorite: true, useAgent: false, jumpHost: 'jump@bastion.example', knownHostsPolicy: 'strict' })
