  ...
  assert.deepEqual(
    {
      ...
      useAgent: updated?.useAgent,
      jumpHost: updated?.jumpHost,
      knownHostsPolicy: updated?.knownHostsPolicy,
      tmuxPath: updated?.tmuxPath,
    },
    {
      groups: ['research', 'gpu'],
      tags: ['production', 'gpu'],
...
test('saves a full host config replacing hosts and credentials', async (t) => {
  ...
  const saved = await hosts.saveHostConfig({
    hosts: {
      version: 2,
      hosts: [{ id: 'alpha', name: 'Alpha', address: 'alpha.example', user: 'guo', port: 22 }],
    } as any,
...
test('updates hosts without touching existing credentials', async (t) => {
  ...
  const updated = await hosts.saveHostConfig({
    hosts: {
      version: 2,
      hosts: [
        { id: 'beta', name: 'Beta', address: 'beta.example', user: 'guo', port: 22 },
        { id: 'alpha', name: 'Alpha', address: 'alpha.example', user: 'guo', port: 22 },
      ],
    } as any,
  })
...
test('rejects an invalid host config structure', async (t) => {
  ...
  await assert.rejects(
    hosts.saveHostConfig({
      hosts: { version: 2, hosts: [{ id: 'local', name: 'Local', address: '127.0.0.1', user: 'guo', port: 22 }] },
    } as any),
    /Invalid host id/,
  )
  await assert.rejects(
    hosts.saveHostConfig({ credentials: { version: 2, credentials: {} } } as any),
