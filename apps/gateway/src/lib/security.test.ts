...
test('reports unsafe gateway deployment settings', () => {
  assert.deepEqual(getSecurityWarnings({ host: '0.0.0.0', authEnabled: false, encryptedTransport: false, tmuxVersion: 'tmux 3.3a' }), [
...
test('does not warn about encrypted loopback deployment with a supported tmux', () => {
  assert.deepEqual(getSecurityWarnings({ host: '127.0.0.1', authEnabled: true, encryptedTransport: true, tmuxVersion: 'tmux 3.7c' }), [])
  assert.deepEqual(getSecurityWarnings({ host: '127.0.0.1', authEnabled: true, encryptedTransport: false, tmuxVersion: 'tmux 3.7c', passwordChangeRequired: true }), ['The default admin password is still active; change it before exposing the Gateway beyond localhost.'])
})
