...
test('parses current tab hook records with optional lifecycle session', () => {
  assert.deepEqual(parseTmuxAgentHookEvent('tmuxgo-agent-monitor:v2\t%1\tdev\tpane-command-finished\t\t0'), {
    paneId: '%1',
    sessionName: 'dev',
  ...
  assert.deepEqual(parseTmuxAgentHookEvent('tmuxgo-agent-monitor:v2\t%1\tpane-exited\t0\t'), {
    paneId: '%1',
    event: 'pane-exited',
    deadStatus: '0',
    commandStatus: '',
