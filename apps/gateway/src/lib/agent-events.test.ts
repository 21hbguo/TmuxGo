...
test('normalizes Claude Notification title, elicitation, and subagent per official docs', () => {
  const notification = normalizeAgentEvent({ hook_event_name: 'Notification', notification_type: 'permission_prompt', title: 'Approve bash', message: 'approval needed' }, { hostId: 'local', provider: 'claude', sessionName: 'dev', tmuxPaneId: '%1' })
  const elicitation = normalizeAgentEvent({ hook_event_name: 'Notification', notification_type: 'elicitation_dialog', message: 'form' }, { hostId: 'local', provider: 'claude', sessionName: 'dev', tmuxPaneId: '%1' })
  const subagentStop = normalizeAgentEvent({ hook_event_name: 'SubagentStop', agent_id: 'sub-1' }, { hostId: 'local', provider: 'claude', sessionName: 'dev', tmuxPaneId: '%1' })
...
test('normalizes Codex official wire format with activeFlags and camelCase status', () => {
  const approval = normalizeAgentEvent({ type: 'thread/status/changed', status: { type: 'active', activeFlags: ['waitingOnApproval'] }, threadId: 'thread-1', eventId: 'approval-2' }, { hostId: 'local', provider: 'codex', tmuxPaneId: '%2', sessionName: 'dev' })
  const input = normalizeAgentEvent({ type: 'thread/status/changed', status: { type: 'active', activeFlags: ['waitingOnUserInput'] }, threadId: 'thread-1', eventId: 'input-2' }, { hostId: 'local', provider: 'codex', tmuxPaneId: '%2', sessionName: 'dev' })
  const systemError = normalizeAgentEvent({ type: 'thread/status/changed', status: { type: 'systemError' }, threadId: 'thread-1', eventId: 'error-2' }, { hostId: 'local', provider: 'codex', tmuxPaneId: '%2', sessionName: 'dev' })
  const closed = normalizeAgentEvent({ method: 'thread/closed', threadId: 'thread-1', eventId: 'closed-2' }, { hostId: 'local', provider: 'codex', tmuxPaneId: '%2', sessionName: 'dev' })
  const autoApproval = normalizeAgentEvent({ type: 'item/autoApprovalReview/started', threadId: 'thread-1', eventId: 'auto-2' }, { hostId: 'local', provider: 'codex', tmuxPaneId: '%2', sessionName: 'dev' })
  assert.equal(approval?.phase, 'permission_required')
...
test('surfaces Claude answer and subagent text as display metadata', () => {
  const stop = normalizeAgentEvent({ hook_event_name: 'Stop', last_assistant_message: 'Fixed the parser' }, { hostId: 'local', provider: 'claude', sessionName: 'dev', tmuxPaneId: '%1' })
  const subagent = normalizeAgentEvent({ hook_event_name: 'SubagentStop', agent_id: 'sub-1', last_assistant_message: 'sub result' }, { hostId: 'local', provider: 'claude', sessionName: 'dev', tmuxPaneId: '%1' })
  assert.equal(stop?.phase, 'idle')
...
test('normalizes OpenCode session errors as failed', () => {
  const sessionError = normalizeAgentEvent({ type: 'session.error', error: { name: 'ProviderAuthError' }, eventId: 'err-1' }, { hostId: 'local', provider: 'opencode', agentSessionId: 'session-1', sessionName: 'dev' })
  const stepFailed = normalizeAgentEvent({ type: 'session.next.step.failed', error: { name: 'UnknownError' }, eventId: 'err-2' }, { hostId: 'local', provider: 'opencode', agentSessionId: 'session-1', sessionName: 'dev' })
...
test('normalizes Reasonix hook lifecycle events per official docs', () => {
  ...
  const approval = normalizeAgentEvent({ event: 'Notification', cwd: '/repo', message: 'approval needed: bash go test ./...' }, ctx)
