...
function normalizeClaude(raw: Record<string, unknown>, context: AgentEventContext, agent: string) {
  ...
  const title = sanitizeMessage(raw.title || data.title) || sanitizeMessage(raw.last_assistant_message || data.last_assistant_message)
  ...
  if (hookEvent === 'permissionrequest' || hookEvent === 'permission_request' || notification === 'permission_prompt') return baseEvent(raw, context, agent, 'permission_required', 'permission_required', 'permission_required', 'hook', { message, ...(display ? { display } : {}) })
  if (notification === 'idle_prompt' || notification === 'agent_needs_input' || notification === 'elicitation_dialog' || hookEvent === 'userinputrequest' || hookEvent === 'user_input_request') return baseEvent(raw, context, agent, 'question_required', 'needs_input', 'question_required', 'hook', { message, ...(display ? { display } : {}) })
  ...
  if (notification === 'agent_completed' || notification === 'elicitation_complete' || notification === 'elicitation_response' || hookEvent === 'stop' || hookEvent === 'completed') return baseEvent(raw, context, agent, 'idle', 'idle', 'completed', 'hook', display ? { display } : {})
  if (hookEvent === 'subagentstart' || hookEvent === 'subagent_start') return baseEvent(raw, context, agent, 'working', 'working', 'started', 'hook')
  if (hookEvent === 'subagentstop' || hookEvent === 'subagent_stop') return baseEvent(raw, context, agent, 'working', 'working', 'started', 'hook', display ? { display } : {})
  return null
}
function normalizeCodex(raw: Record<string, unknown>, context: AgentEventContext, agent: string) {
  ...
  const flagsValue = object(raw.status).activeFlags || object(data.status).activeFlags || raw.activeFlags || data.activeFlags
  ...
  const waitingApprovalEvent = eventName.includes('permission') || (eventName.includes('approval') && !eventName.includes('autoapproval'))
  const waitingInputEvent = eventName.includes('user_input') || eventName.includes('userinput')
  if (eventName.includes('process_exited') || eventName.includes('processexited') || eventName === 'process_exit' || eventName === 'thread_closed') return baseEvent(raw, context, agent, 'process_exited', 'ended', 'ended')
  if (waitingApproval || waitingApprovalEvent) return baseEvent(raw, context, agent, 'permission_required', 'permission_required', 'permission_required')
  if (waitingInput || waitingInputEvent) return baseEvent(raw, context, agent, 'question_required', 'needs_input', 'question_required')
  if (status === 'systemerror' || status === 'system_error' || eventName.includes('failure') || eventName.includes('error')) return baseEvent(raw, context, agent, 'failed', 'failed', 'failed', undefined, { message: sanitizeMessage(raw.message || data.message) })
  if (status === 'active' || status === 'working' || eventName.endsWith('_started')) return baseEvent(raw, context, agent, 'working', 'working', 'started')
  if (status === 'idle' || status === 'notloaded' || eventName.includes('item_completed') || eventName.includes('turn_completed') || eventName.includes('hook_completed')) return baseEvent(raw, context, agent, 'idle', 'idle', 'completed')
  return null
}
function normalizeOpenCode(raw: Record<string, unknown>, context: AgentEventContext, agent: string) {
  ...
  if (eventName.includes('error') || eventName.includes('failed') || eventName.includes('fail')) return baseEvent(raw, context, agent, 'failed', 'failed', 'failed', undefined, { message: sanitizeMessage(raw.message || data.message) })
  if (eventName === 'session.next.text.ended' || eventName === 'session.next.reasoning.ended') {
    const text = sanitizeMessage(data.text || raw.text)
    return baseEvent(raw, context, agent, 'working', 'working', 'started', undefined, text ? { display: { title: text.slice(0, 160), ttlMs: 60000 } } : {})
  }
...
function normalizeReasonix(raw: Record<string, unknown>, context: AgentEventContext, agent: string) {
  ...
  if (eventName === 'sessionstart') return baseEvent(raw, context, agent, 'session_started', 'working', 'started', 'hook')
  if (eventName === 'sessionend') return baseEvent(raw, context, agent, 'session_ended', 'ended', 'ended', 'hook')
  if (eventName === 'subagentstop') return baseEvent(raw, context, agent, 'working', 'working', 'started', 'hook', display ? { display } : {})
  if (eventName === 'stop') return baseEvent(raw, context, agent, 'idle', 'idle', 'completed', 'hook', display ? { display } : {})
  if (eventName === 'notification') {
    if (message && /question|input|answer|respond/i.test(message)) return baseEvent(raw, context, agent, 'question_required', 'needs_input', 'question_required', 'hook', { message })
    return baseEvent(raw, context, agent, 'permission_required', 'permission_required', 'permission_required', 'hook', { message })
  }
  if (eventName === 'pretooluse' || eventName === 'posttooluse' || eventName === 'userpromptsubmit' || eventName === 'postllmcall' || eventName === 'precompact') return baseEvent(raw, context, agent, 'working', 'working', 'started', 'hook')
  return null
}
...
export function normalizeAgentEvent(input: unknown, context: AgentEventContext) {
  ...
  const normalized = provider === 'claude' ? normalizeClaude(raw, context, agent) : provider === 'codex' ? normalizeCodex(raw, context, agent) : provider === 'opencode' ? normalizeOpenCode(raw, context, agent) : provider === 'reasonix' ? normalizeReasonix(raw, context, agent) : normalizeGeneric(raw, context, agent)
