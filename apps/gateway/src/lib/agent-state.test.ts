...
test('detects agents from pane child processes', () => {
  assert.equal(
    detectProcessAgent(
      'node /home/guo/.nvm/versions/node/v22.22.0/bin/codex --dangerously-bypass-approvals-and-sandbox',
    ),
    'codex',
  )
...
test('detects reasonix working state from in-pane braille spinner', () => {
  assert.deepEqual(
    detectAgentEvidence(
      'node /home/guo/.nvm/versions/node/v22.22.0/bin/reasonix',
      '',
  ...
  assert.deepEqual(
    detectAgentEvidence('node /home/guo/.nvm/versions/node/v22.22.0/bin/reasonix', '', '⎿  ⠙ 运行中 · 0 秒', 'reasonix')
      ?.phase,
    'working',
  )
})
