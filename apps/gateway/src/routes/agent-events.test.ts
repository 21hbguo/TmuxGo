...
test('protects HTTP Agent events and accepts event or payload once', async (t) => {
  ...
  const body = {
    ...
    paneId: 'local:%1',
    sessionName: 'dev',
    payload: { type: 'busy', eventId: 'busy-1' },
  }
  ...
  const accepted = await fastify.inject({
    method: 'POST',
    url: '/agent-events',
    headers: { authorization: 'Bearer agent-event-secret' },
    payload: body,
  })
  ...
  const duplicate = await fastify.inject({
    method: 'POST',
    url: '/agent-events',
    headers: { 'x-tmuxgo-agent-token': 'agent-event-secret' },
    payload: body,
  })
  ...
  const both = await fastify.inject({
    ...
    url: '/agent-events',
    headers: { 'x-tmuxgo-agent-token': 'agent-event-secret' },
    payload: { ...body, event: { type: 'busy' } },
  })
  assert.equal(both.statusCode, 400)
  const foreignPane = await fastify.inject({
    method: 'POST',
    url: '/agent-events',
    headers: { 'x-tmuxgo-agent-token': 'agent-event-secret' },
    payload: { ...body, paneId: 'other:%1' },
  })
