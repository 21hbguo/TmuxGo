...
test('persists VAPID keys and synchronizes unread notifications across devices', async (t) => {
  ...
  const notification = await persistAgentNotification({ type: 'agent_notification', hostId: 'local', sessionName: 'dev', eventId: 'event-1', pane: { paneId: 'local:%1', tmuxPaneId: '%1', sessionName: 'dev', agent: 'codex', agentStatus: 'blocked', phase: 'permission_required', lastEvent: 'permission_required', revision: 1 } })
  assert.equal(notification?.status, 'permission_required')
  const unreadDeviceOne = await fastify.inject({ method: 'GET', url: '/agent-notifications/unread?deviceId=device-one' })
  ...
  const marked = await fastify.inject({ method: 'POST', url: '/agent-notifications/read', payload: { deviceId: 'device-one', ids: ['event-1'] } })
  ...
  const unreadDeviceTwo = await fastify.inject({ method: 'GET', url: '/agent-notifications/unread?deviceId=device-two' })
  assert.equal((unreadDeviceTwo.json() as { notifications: { id: string }[] }).notifications[0]?.id, 'event-1')
  const mismatch = await fastify.inject({ method: 'GET', url: '/agent-notifications/unread?deviceId=device-one', headers: { 'x-tmuxgo-device-id': 'device-two' } })
  assert.equal(mismatch.statusCode, 400)
  const invalidSubscription = await fastify.inject({ method: 'POST', url: '/agent-notifications/subscriptions', payload: { deviceId: 'device-one', subscription: { endpoint: 'http://not-secure.example', keys: { p256dh: 'key', auth: 'auth' } } } })
  assert.equal(invalidSubscription.statusCode, 400)
  const registered = await fastify.inject({ method: 'POST', url: '/agent-notifications/subscriptions', payload: { deviceId: 'device-one', subscription: { endpoint: 'https://push.example.test/subscription-1', keys: { p256dh: 'key', auth: 'auth' } } } })
  assert.equal(registered.statusCode, 200)
  const subscriptionId = (registered.json() as { id: string }).id
  const revoked = await fastify.inject({ method: 'DELETE', url: `/agent-notifications/subscriptions/${subscriptionId}?deviceId=device-one` })
