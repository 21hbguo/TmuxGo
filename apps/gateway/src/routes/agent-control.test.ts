...
test('protects control routes with token and TMUXGO_ENV guard', async (t) => {
  ...
  const noGuard = await fastify.inject({ method: 'POST', url: '/v1/control/panes/split', headers: { 'x-tmuxgo-agent-token': 'agent-control-secret' }, payload: body })
  assert.equal(noGuard.statusCode, 403)
  const invalidWait = await fastify.inject({ method: 'POST', url: '/v1/control/agent/wait', headers: { 'x-tmuxgo-agent-token': 'agent-control-secret', 'x-tmuxgo-env': '1' }, payload: { target: { paneId: 'local:%1' }, condition: {} } })
